...
<xsl:template name="gen_index">
  ...
  <xsl:for-each select="$navData//node">
    ...
    <xsl:if test="parent::node()[name()='node']">
      <xsl:text disable-output-escaping="yes">&#x20;&#x20;&#x20;</xsl:text>
      <!-- xsl:value-of select="count(ancestor::node)"/-->
      <!-- xsl:value-of select="substring('++++++++++',1,count(ancestor::node))"/-->
      <xsl:value-of select="substring('|_________',1,count(ancestor::node) +1)"/>
    </xsl:if>
    <xsl:text disable-output-escaping="yes">&#x20;&lt;a href="#</xsl:text>
      <!-- <xsl:value-of select="@name"/>-->
