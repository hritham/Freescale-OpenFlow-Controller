...
void* of_flow_actionset_starttrans(struct cm_array_of_iv_pairs   * key_iv_pairs,
                                   uint32_t command_id,
                                   struct cm_app_result ** result_pp);

int32_t  of_flow_actionset_endtrans(void *config_trans_p,
                                    uint32_t command_id,
                                    struct cm_app_result ** result_pp);

int32_t of_flow_actionset_addrec(void * config_trans_p,
...
int32_t of_flow_actionset_delrec(void * config_trans_p,
    struct cm_array_of_iv_pairs * key_iv_pairs,
    struct cm_app_result ** result_pp);

int32_t of_flow_actionset_verifycfg(struct cm_array_of_iv_pairs * key_iv_pairs,
...
int32_t of_flow_actionset_ucm_setmandparams (struct cm_array_of_iv_pairs *
    mand_iv_pairs,
...
int32_t of_flow_actionset_ucm_setoptparams (struct cm_array_of_iv_pairs *
    opt_iv_pairs,
    struct ofi_flow_action *info,
    struct cm_app_result ** result_pp);

int32_t of_flow_action_getexactrec(struct cm_array_of_iv_pairs * key_iv_pairs,
    struct cm_array_of_iv_pairs ** result_iv_pairs_pp);

struct cm_dm_app_callbacks of_flow_actionset_ucm_callbacks = 
{
	of_flow_actionset_starttrans,
	of_flow_actionset_addrec,
	of_flow_actionset_setrec,
	of_flow_actionset_delrec,
	of_flow_actionset_endtrans,
	NULL,
	NULL,
	of_flow_action_getexactrec,
	of_flow_actionset_verifycfg,
	NULL
};
...
int32_t of_flow_actionset_appl_ucmcbk_init (void)
{
	...
	return_value = cm_register_app_callbacks(
                      CM_ON_DIRECTOR_DATAPATH_FLOWMOD_ACTIONSET_APPL_ID,
                      &of_flow_actionset_ucm_callbacks);
  if(return_value != OF_SUCCESS)
  {
    cntrlrucm_debugmsg("flow_actionset application CBK registration failed");
    return OF_FAILURE;
  }
		...
		uint32_t command_id, struct cm_app_result ** result_pp)
{
	...
	return_value = 
      of_flow_actionset_ucm_validatemandparams (key_iv_pairs, &of_flow_actionset_result);
	if (return_value != OF_SUCCESS)
	{
                                   ...
                                   struct cm_app_result ** result_pp)
{
 ...
 if ((of_flow_actionset_ucm_setmandparams(key_iv_pairs, NULL,
                                      &flow_info, &flow_action_info,
	...
	OF_LIST_SCAN(flow_trans_list_g, trans_rec, struct flow_trans *,offset)
	{
		...
		{
			CM_CBK_DEBUG_PRINT ("transflow_actionset found");
			break;
		}
                                 ...
                                 struct cm_app_result ** result_pp)
{
  ...
  if ((of_flow_actionset_ucm_setmandparams(mand_iv_pairs,NULL,
                   &flow_info, flow_action_info,
                   &of_flow_actionset_result)) != OF_SUCCESS)
	{
		CM_CBK_DEBUG_PRINT ("Set Mandatory Parameters Failed");
	...
	return_value = of_flow_actionset_ucm_setoptparams (opt_iv_pairs, flow_action_info, 
	...
	return_value = of_flow_add_action_to_list(trans_rec->datapath_handle,
                              flow_action_info, flow_info.flow_id);

	if (return_value != OF_SUCCESS)
	{
      CM_CBK_DEBUG_PRINT ("flow_actionset Addition to flow %d Failed", flow_action_info->action_type);
      fill_app_result_struct (&of_flow_actionset_result, NULL, CM_GLU_VLAN_FAILED);
      *result_pp =of_flow_actionset_result;
      return OF_FAILURE;
	}
		...
		struct cm_app_result ** result_pp)
{
	...
	return_value = of_flow_remove_action_from_list(trans_rec->datapath_handle,
                                        flow_info.flow_id,
                                        flow_action_info.action_id);
	if (return_value != OF_SUCCESS)
	{
      CM_CBK_DEBUG_PRINT("Error: flow_actionset does not exist with id %d",
 ...
 * Result:
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int32_t of_flow_actionset_ucm_setmandparams (struct cm_array_of_iv_pairs *mand_iv_pairs,
                               struct dprm_datapath_general_info *datapath_info,
                               struct  ofi_flow_mod_info *flow_info,
                               struct ofi_flow_action *flow_action_info,
                               struct cm_app_result ** result_pp)
{
  ...
  CM_CBK_DEBUG_PRINT ("Entered");
  for (opt_param_cnt = 0; opt_param_cnt < mand_iv_pairs->count_ui; opt_param_cnt++)
  {
      switch (mand_iv_pairs->iv_pairs[opt_param_cnt].id_ui)
      {
         ...
         case CM_DM_FLOWMOD_FLOWID_ID:
            ...
            break;
         case CM_DM_ACTIONSET_ACTIONSETID_ID:
            flow_action_info->action_id = 
            of_atoi((char *) mand_iv_pairs->iv_pairs[opt_param_cnt].value_p);
            break;
         case  CM_DM_ACTIONSET_ACTIONSETTYPE_ID:			
            ...
            CM_CBK_DEBUG_PRINT ("flow_actionset type %d %d ",
                                flow_action_info->action_type,flow_action_type);
            break;
       }
  }
 ...
 * Result:
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
 int32_t of_flow_actionset_ucm_validatemandparams(struct cm_array_of_iv_pairs *
                                                  mand_iv_pairs,
                                                  struct cm_app_result ** result_pp)
{
 uint32_t opt_param_cnt;
 uint32_t flow_id;
 uint32_t flow_action_id;
 struct cm_app_result *of_flow_actionset_result=NULL;
			...
			opt_param_cnt++)
	{
      ...
      case CM_DM_ACTIONSET_ACTIONSETID_ID:
			     flow_action_id = of_atoi((char *) mand_iv_pairs->iv_pairs[opt_param_cnt].value_p);
           CM_CBK_DEBUG_PRINT ("flow_action_type is %d", flow_action_id);
           break;
		}
	}
	CM_CBK_PRINT_IVPAIR_ARRAY (mand_iv_pairs);
 ...
 * Result:
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int32_t of_flow_actionset_ucm_setoptparams(struct cm_array_of_iv_pairs *opt_iv_pairs,
                                           struct ofi_flow_action *flow_action_info,
                                           struct cm_app_result ** result_pp)
{
	uint32_t opt_param_cnt;
	...
	for (opt_param_cnt = 0; opt_param_cnt < opt_iv_pairs->count_ui; opt_param_cnt++)
	{
         ...
         {          
             case CM_DM_ACTIONSET_ACTIONSETDATA_ID: 
                  ...
                  CM_CBK_DEBUG_PRINT("flow_actionset type %d ",
                                      flow_action_info->action_type);
                  break;
             default:
                   break;
...
static int32_t 
of_flow_action_ucm_getparams(uint32_t flow_id, 
...
//#define CM_FLOWMOD_CHILD_COUNT 1
  ...
  FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],CM_DM_ACTIONSET_ACTIONSETID_ID,
                  CM_DATA_TYPE_STRING, string);
  index++;
                                   ...
                                   struct cm_array_of_iv_pairs ** result_iv_pairs_pp)
{
  ...
  CM_CBK_DEBUG_PRINT ("Entered");
  if (of_flow_actionset_ucm_setmandparams(key_iv_pairs, &datapath_info, &flow_info,
                                          &action_info, &flow_result) != OF_SUCCESS)
  {
     CM_CBK_DEBUG_PRINT ("Set Mandatory Parameters Failed");
     return OF_FAILURE;
  }
  ...
  return_value = of_get_flow_action(datapath_handle, flow_info.flow_id,
                                    action_info.action_id, &action_entry_p);
  if ( return_value != OF_SUCCESS)
  {
    CM_CBK_DEBUG_PRINT ("of_get_flow action failed");
    return OF_FAILURE;
  }
  ...
  return_value=of_flow_action_ucm_getparams(flow_info.flow_id,
                                            action_entry_p, &result_iv_pairs_p[0]);
  if (return_value != OF_SUCCESS)
  {
    CM_CBK_DEBUG_PRINT ("of_flow_action_ucm_getparams failed");
    return OF_FAILURE;
  } 
