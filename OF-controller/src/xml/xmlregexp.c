...
#ifndef INT_MAX
#define INT_MAX 2147483647 /* easy to flag and big enough for our needs */
#endif
...
xmlRegAtomAddRange(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom,
    ...
    if (atom == NULL) {
	XML_ERROR("add range: atom is NULL");
	return;
    }
    if (atom->type != XML_REGEXP_RANGES) {
	XML_ERROR("add range: atom is not ranges");
	return;
    }
    if (atom->maxRanges == 0) {
	atom->maxRanges = 4;
...
xmlRegAtomPush(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
    if (atom == NULL) {
	XML_ERROR("atom push: atom is NULL");
	return(-1);
    }
    if (ctxt->maxAtoms == 0) {
	ctxt->maxAtoms = 4;
...
xmlRegStateAddTrans(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state,
    ...
    if (state == NULL) {
	XML_ERROR("add state: state is NULL");
	return;
    }
    if (target == NULL) {
	XML_ERROR("add state: target is NULL");
	return;
    }
    /*
...
xmlFAGenerateTransitions(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr from,
    ...
    if (atom == NULL) {
	XML_ERROR("genrate transition: atom == NULL");
	return(-1);
    }
	...
	for (;exec->transno < exec->state->nbTrans;exec->transno++) {
		...
		if ((value == NULL) && (final)) {
			...
			if ((count >= counter->min) &&
			    (count < counter->max) && (t->atom))
                            {
                            if(xmlStrEqual(value, t->atom->valuep)) {
			    ret = 1;
			    break;
			}
                          }
...
xmlFAParseCharProp(xmlRegParserCtxtPtr ctxt) {
	...
	XML_ERROR("Unknown char property");
	return;
    }
    if (ctxt->atom == NULL) {
...
xmlFAParseCharClassEsc(xmlRegParserCtxtPtr ctxt) {
    ...
    if (CUR != '\\') {
	XML_ERROR("Escaped sequence: expecting \\");
	return;
    }
    NEXT;
    cur = CUR;
...
/*		else if ((cur >= 'a') && (cur <= 'f'))
	    ...
	    XML_ERROR("Char ref: expecting [0-9A-F]");
	    return(-1);
	}
    } else {
	if ((cur >= '0') && (cur <= '9')) {
	    ...
	    XML_ERROR("Char ref: expecting [0-9]");
	    return(-1);
	}
    }
    if (cur != ';') {
	XML_ERROR("Char ref: expecting ';'");
	return(-1);
    } else {
	NEXT;
    }
...
xmlFAParseCharRange(xmlRegParserCtxtPtr ctxt) {
	...
	switch (cur) {
		...
		start = cur; break;
	    default:
		XML_ERROR("Invalid escape value");
		return;
	}
	end = start;
        len = 1;
	...
	XML_ERROR("Expecting a char range");
	return;
    }
    XML_NEXTL(len);
    if (start == '-') {
	...
	XML_ERROR("Expecting the end of a char range");
	return;
    }
    XML_NEXTL(len);
    /* TODO check that the values are acceptable character ranges for XML */
    if (end < start) {
	XML_ERROR("End of range is before start of range");
    } else {
        xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg,
		           XML_REGEXP_CHARVAL, start, end, NULL);
    }
...
xmlFAParseCharGroup(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == '^') {
		...
		XML_ERROR("charClassExpr: ']' expected");
		break;
	    }
	    ctxt->neg = neg;
	    break;
...
xmlFAParseCharClass(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == ']') {
	    ...
	    XML_ERROR("xmlFAParseCharClass: ']' expected");
	}
    } else {
	xmlFAParseCharClassEsc(ctxt);
    }
...
xmlFAParseQuantifier(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == '}') {
	    ...
	    XML_ERROR("Unterminated quantifier");
	}
	if (max == 0)
	    max = min;
	if (ctxt->atom != NULL) {
...
xmlFAParseAtom(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == ')') {
	    ...
	    XML_ERROR("xmlFAParseAtom: expecting ')'");
	}
...
xmlFAParsePiece(xmlRegParserCtxtPtr ctxt) {
	...
	return(0);
    if (ctxt->atom == NULL) {
	XML_ERROR("internal: no atom generated");
    }
...
xmlRegexpCompile(const xmlChar *regexp) {
    ...
    if (CUR != 0) {
	XML_ERROR("xmlFAParseRegExp: extra characters");
    }
    ctxt->end = ctxt->state;
