...
#define IS_IXMLSCHEMA(node, type)					\
   ((node != NULL) && (node->namespaceURI != NULL) &&				\
	(node->localName != NULL) && \
    (xmlStrEqual(node->localName, (const xmlChar *) type)) &&		\
    (xmlStrEqual(node->namespaceURI, xmlSchemaNs)))
...
xmlSchemaValidateElementByWildcard(xmlSchemaValidCtxtPtr ctxt,
				   xmlSchemaTypePtr type);
static int
xmlSchemaHasElemOrCharContent(IXML_Node *node);
#if 0
static int
 ...
 */
static void
xmlSchemaErr(xmlSchemaParserCtxtPtr ctxt)
{
    if (ctxt != NULL)
        ctxt->nberrors++;
...
xmlSchemaPErrMemory(xmlSchemaParserCtxtPtr ctxt,
                    const char *extra, IXML_Node *node)
{
    if (ctxt != NULL)
        ctxt->nberrors++;
 ...
 */
static void
xmlSchemaPErr(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node, int error,
 ...
 */
static void
xmlSchemaPErr2(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node,
               IXML_Node *child, int error,
               const char *msg, const xmlChar * str1, const xmlChar * str2)
{
    if (child != NULL)
 ...
 */
static void
xmlSchemaPErrExt(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node, int error,
 ...
 */
static void
xmlSchemaVErr3(xmlSchemaValidCtxtPtr ctxt, IXML_Node *node, int error,
               const char *msg, const xmlChar *str1, const xmlChar *str2,
	       const xmlChar *str3)
{
/*    xmlStructuredErrorFunc schannel = NULL;
 ...
 */
static void
xmlSchemaVErrExt(xmlSchemaValidCtxtPtr ctxt, IXML_Node *node, int error,
...
#ifdef XMLDEBUG
	 printf(msg, str1, str2, str3, str4, str5);
#endif
/*        channel = ctxt->error;
        schannel = ctxt->serror;
        data = ctxt->userData;*/
 ...
 */
static void
xmlSchemaVErr(xmlSchemaValidCtxtPtr ctxt, IXML_Node *node, int error,
              const char *msg, const xmlChar * str1, const xmlChar * str2)
{
/*    xmlStructuredErrorFunc schannel = NULL;
    xmlGenericErrorFunc channel = NULL;
 ...
 */
static const xmlChar*
xmlSchemaFormatNsPrefixLocal(xmlChar **buf,  IXML_Node *node)
{
    if (*buf != NULL) {
	xmlFree(*buf);
	*buf = NULL;
	...
	return(node->nodeName);
	    ...
	    *buf = xmlStrdup(ixmlNode_getPrefix(node));
	    *buf = xmlStrcat(*buf, BAD_CAST ":");
	    *buf = xmlStrcat(*buf,ixmlNode_getLocalName(node));
    }
		     ...
		     int parsing)
{
	...
	*buf = xmlStrdup(itemDes);
	...
	if (itemNode->nodeType == eATTRIBUTE_NODE)
		elem = &((((IXML_Attr *)itemNode)->ownerElement)->n);
	else
	    elem = itemNode;
	*buf = xmlStrdup(BAD_CAST "Element '");
	if (parsing)
	    *buf = xmlStrcat(*buf, elem->nodeName);
	else
	    *buf = xmlStrcat(*buf,
		xmlSchemaFormatNsPrefixLocal(&str, elem));
	*buf = xmlStrcat(*buf, BAD_CAST "'");
    }
    if ((itemNode != NULL) && (itemNode->nodeType == eATTRIBUTE_NODE)) {
	*buf = xmlStrcat(*buf, BAD_CAST ", attribute '");
	*buf = xmlStrcat(*buf, xmlSchemaFormatNsPrefixLocal(&str, itemNode));
	*buf = xmlStrcat(*buf, BAD_CAST "'");
    }
    FREE_AND_NULL(str);
		   ...
		   const xmlChar *str3)
{
	...
	*/
	if ((facetType == XML_SCHEMA_FACET_LENGTH) ||
	    ...
	    * this value?
	    */
	    if (node->nodeType == eATTRIBUTE_NODE)
		msg = xmlStrcat(msg, BAD_CAST "The value '%s' has a length of '%s'; ");
	    else
		msg = xmlStrcat(msg, BAD_CAST "The value has a length of '%s'; ");

	    T_snprintf(len, 24, "%lu", xmlSchemaGetFacetValueAsULong(facet));
	    T_snprintf(actLen, 24, "%lu", length);
			 ...
			 const char *message)
{
    ...
    {
		printf("%s: The attribute '%s' is required but missing.\n",
			 ...
			 const char *refTypeStr)
{
	...
	printf("%s, attribute '%s': The QName value %s does not resolve to a(n) "
	    "%s.\n", BAD_CAST des, BAD_CAST name,
			...
			const char *msg)
{
    ...
    if (ownerDes == NULL)
	xmlSchemaFormatItemForReport(&des, NULL, ownerItem, attr->parentNode, 1);
    else if (*ownerDes == NULL) {
	xmlSchemaFormatItemForReport(ownerDes, NULL, ownerItem, attr->parentNode, 1);
	des = *ownerDes;
    } else
	des = *ownerDes;
	printf("%s, attribute '%s': %s.\n",BAD_CAST des, attr->nodeName, (const xmlChar *) msg);
/*    xmlSchemaPErrExt(ctxt,  attr, error, NULL, NULL, NULL,
	"%s, attribute '%s': %s.\n",
	BAD_CAST des, attr->nodeName, (const xmlChar *) msg, NULL, NULL);*/
    if (ownerDes == NULL)
	FREE_AND_NULL(des);
			 ...
			 IXML_Node *attr)
{
	...
	des = *ownerDes;
	printf("%s: The attribute '%s' is not allowed.\n", BAD_CAST des,
		   xmlSchemaFormatNsPrefixLocal(&strA, attr));
/*    xmlSchemaPErr(ctxt, attr, error,
		    ...
		    const xmlChar *str3)
{
	...
	itemElem = item->node;
	printf((const char *) msg, BAD_CAST des, str1, str2, str3);
		    ...
		    const xmlChar *str1)
{
	...
	printf((const char *) msg, BAD_CAST des, BAD_CAST strA, str1);
/*    xmlSchemaPErrExt(ctxt, itemElem, error, NULL, NULL, NULL,
			  ...
			  xmlSchemaFacetPtr facet)
{
    ...
    printf("%s: The facet '%s' is not allowed on types derived from the "
	"type %s.\n",
			 ...
			 const char *name2)
{
	...
	des = *ownerDes;
	printf("%s: The attributes '%s' and '%s' are mutually exclusive.\n",
		   BAD_CAST des, BAD_CAST name1, BAD_CAST name2);
/*    xmlSchemaPErrExt(ctxt,  attr, error, NULL, NULL, NULL,
			...
			const xmlChar *str2)
{
	...
	*/
	if (node->nodeType == eATTRIBUTE_NODE) {
	printf("xmlSchemaPSimpleTypeErr:%s, attribute '%s' [%s]: The value '%s' is not "
		"valid.\n",BAD_CAST des, xmlSchemaFormatNsPrefixLocal(&strA,
		node), BAD_CAST typeDes, value);
	    /*xmlSchemaPErrExt(ctxt, node, error, NULL, NULL, NULL,
		"%s, attribute '%s' [%s]: The value '%s' is not "
		"valid.\n",
		BAD_CAST des, xmlSchemaFormatNsPrefixLocal(&strA,
					node), BAD_CAST typeDes, value, NULL);*/
	} else {
	printf("xmlSchemaPSimpleTypeErr:%s [%s]: The character content is not valid.\n",
...
/*	    xmlSchemaPErr(ctxt, node, error,
	...
	printf((const char *) msg,
		BAD_CAST des, xmlSchemaFormatNsPrefixLocal(&strA,
					node), BAD_CAST typeDes, str1, str2);
/*	    xmlSchemaPErrExt(ctxt, node, error, NULL, NULL, NULL,
		(const char *) msg,
		BAD_CAST des, xmlSchemaFormatNsPrefixLocal(&strA,
					node), BAD_CAST typeDes, str1, str2);*/
	} else {
	printf((const char *) msg,BAD_CAST des, BAD_CAST typeDes, str1, str2);
/*	    xmlSchemaPErrExt(ctxt, node, error, NULL, NULL, NULL,
		(const char *) msg,
...
/*	xmlSchemaPErr2(ctxt, ownerElem, child, error,
	...
	if (content != NULL) {
		printf("%s: The content is not valid. Expected is %s.\n",
			   BAD_CAST des, BAD_CAST content);
/*	    xmlSchemaPErr2(ctxt, ownerElem, child, error,
		"%s: The content is not valid. Expected is %s.\n",
		BAD_CAST des, BAD_CAST content);*/
	} else {
			printf("%s: The content is not valid.\n",BAD_CAST des);
/*	    xmlSchemaPErr2(ctxt, ownerElem, child, error,
			 ...
			 IXML_Node *attr)
{
    ...
    xmlSchemaVErr(ctxt, attr,
	error,
	/* XML_SCHEMAS_ERR_ATTRUNKNOWN, */
	"%s: The attribute '%s' is not allowed.\n",
	xmlSchemaFormatItemForReport(&strE, NULL, NULL, attr->parentNode, 0),
		xmlSchemaFormatNsPrefixLocal(&strA, attr));
    FREE_AND_NULL(strE)
    FREE_AND_NULL(strA)
}
		    ...
		    const xmlChar *str1)
{
    ...
    if (node->nodeType != eDOCUMENT_NODE) {
	xmlSchemaFormatItemForReport(&msg, NULL, NULL, node, 0);
	if (type != NULL) {
	    msg = xmlStrcat(msg, BAD_CAST " [");
	    msg = xmlStrcat(msg, xmlSchemaFormatItemForReport(&str, NULL, type, NULL, 0));
...
/*        xmlSchemaPErrMemory(ctxt, "allocating schema", NULL);*/
	printf("xmlSchemaNewSchema:Error allocating schema\n");
#endif
        return (NULL);
    }
...
xmlSchemaNewAnnot(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node)
{
...
xmlSchemaAnnotDump(FILE * output, xmlSchemaAnnotPtr annot)
{
	...
	content = ixmlNode_getNodeValueEx(annot->content);
...
static IXML_Node*
ixmlSchemaGetPropNode(IXML_Node *node, const char *name)
{
    IXML_Node *prop;
    ...
    pAttribs = ixmlNode_getAttributes(node);
	if (!pAttribs)
	{
		return NULL;
	}
    ...
    for (i=0; i<ulAttrCount; i++)
    {
		...
		if (xmlStrEqual(ixmlNode_getNodeName(prop),BAD_CAST name))
		{
 ...
 */
static IXML_Node*
xmlSchemaGetPropNodeNs(IXML_Node* node, const char *uri, const char *name)
{
    /*xmlAttrPtr prop;*/
    int ulAttrCount = 0, j = 0;

    if ((node == NULL) || (name == NULL))
	return(NULL);

	IXML_NamedNodeMap *pAttribs = NULL;
	IXML_Node *pAttribN = NULL;
	...
	for (j=0; j<ulAttrCount; j++)
	{
	   pAttribN = ixmlNamedNodeMap_item(pAttribs,j);

	   if (xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST name) &&
	    		xmlStrEqual(pAttribN->namespaceURI, BAD_CAST uri))
	   {
			ixmlNamedNodeMap_free(pAttribs);
			return (pAttribN);
	   }
...
#if 0
static const xmlChar *
xmlSchemaGetNodeContent(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node)
{
    return(ixmlNode_getNodeValueEx(node));
}
#endif
/**
 * xmlSchemaGetProp:
 ...
 */
static const xmlChar *
xmlSchemaGetProp(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node,
                 const char *name)
{
    IXML_Node *pAttribN = NULL;
    int ulAttrCount = 0, i = 0;
    IXML_NamedNodeMap *pAttribs = NULL;
   ...
   for (i=0; i<ulAttrCount; i++)
   {
   	pAttribN = ixmlNamedNodeMap_item(pAttribs,i);

    if (xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST name))
	{
	    ixmlNamedNodeMap_free(pAttribs);
		return(ixmlNode_getNodeValueEx(pAttribN));
	}
   }
...
#define IS_BLANK_NODE(n)						\
    (((n)->nodeType == eTEXT_NODE) && (xmlSchemaIsBlank((n)->nodeValue)))
			   ...
			   xmlSchemaTypePtr item)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddAssembledItem:Error allocating new item buffer\n");
#endif		
/*	    xmlSchemaPErrMemory(ctxt,
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddAssembledItem:Error growing item buffer\n");
#endif
/*	    xmlSchemaPErrMemory(ctxt,
                     ...
                     const xmlChar * name)
{
	...
	if (ret->name)
	{
		strcpy((char*)ret->name,(char*) name);
	}
	else
	{
		return NULL;
	...
	*/
	/*xmlSchemaPErr(ctxt, ctxt->doc,
		      XML_SCHEMAP_REDEFINED_NOTATION,
                      "Annotation declaration '%s' is already declared.\n",
                      name, NULL);*/
		printf("\n**** Parser error: Annotation declaration '%s' is already declared\n", name);
        xmlFree(ret);
        return (NULL);
    }
    return (ret);
		      ...
		      IXML_Node *node)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddAttribute:Error allocating attribute\n");
#endif
...
/*    ret->targetNamespace = xmlDictLookup(ctxt->dict, namespace, -1);*/
	if (namespace)
	{
		strcpy((char*)ret->targetNamespace,(char*)namespace);
	}
	else
	{
		ret->targetNamespace = NULL;
	}
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddAttribute:A global attribute declaration with the name '%s' does already exist\n", name);
#endif
		    ...
		    IXML_Node *node, int topLevel)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddElement:Error allocating element\n");
#endif
...
/*        xmlSchemaPErrMemory(ctxt, "allocating element", NULL);*/
	...
	if (namespace != NULL)
	{
		ret->targetNamespace = (char*)malloc(strlen(namespace));
		if(ret->targetNamespace)
			strcpy((char *)ret->targetNamespace, (char *)namespace);
	}
	else
	{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddElement:A global element declaration with" 
	 	   "the name '%s' does already exist", name);
#endif
...
/*	    xmlSchemaPCustomErr(ctxt,
	    ...
	    T_snprintf(buf, 29, "#eCont %d", ctxt->counter++ + 1);
	    val = xmlHashAddEntry3(schema->elemDecl, name, (xmlChar *) buf,
		namespace, ret);
	    if (val != 0) {
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf ("Internal error: xmlSchemaAddElement,"
		 ...
		 IXML_Node *node)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddType:Error allocating type\n");
#endif
/*        xmlSchemaPErrMemory(ctxt, "allocating type", NULL);*/
   ...
   /* ret->name = xmlDictLookup(ctxt->dict, name, -1);*/
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddType:A global type definition with the"
...
/*	    xmlSchemaPCustomErr(ctxt,
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("Internal error: xmlSchemaAddType, on type '%s'\n", name);
#endif
...
xmlSchemaAddGroup(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
                  const xmlChar * name, IXML_Node *node)
{
    xmlSchemaTypePtr ret = NULL;
    int val;
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddGroup:Error adding group\n");
#endif
...
/*        xmlSchemaPErrMemory(ctxt, "adding group", NULL);*/
    ...
    val = xmlHashAddEntry2(schema->groupDecl, name,
						   schema->targetNamespace,ret);
    if (val != 0) {
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddGroup:A global model group definition "
		   "with the name '%s' does already exist", name);
#endif
	/*xmlSchemaPCustomErr(ctxt,
...
xmlSchemaNewWildcardNsConstraint(xmlSchemaParserCtxtPtr ctxt)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaNewWildcardNsConstraint:creating wildcard namespace constraint\n");
#endif
...
xmlSchemaAddWildcard(xmlSchemaParserCtxtPtr ctxt)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaAddWildcard:adding wildcard\n");
#endif
 ...
 */
static const xmlChar *
xmlGetQNameProp(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node,
                const char *name, const xmlChar ** namespace)
{
    ...
    *namespace = NULL;
    attr = ixmlSchemaGetPropNode(node, name);
    if (attr == NULL)
	return (NULL);
    /*val = xmlSchemaGetNodeContent(ctxt, (xmlNodePtr) attr);*/
    val = ixmlNode_getNodeValueEx(attr);
    ...
    if (!strchr((char *) val, ':')) 
    {
	ixmlSearchNs(node->ownerDocument, node, NULL, namespace);
	if (*namespace == NULL) 
	{
	    return (val);
	}
    ...
    ret = ixmlSplitQName2(val, &prefix);
    if (ret == NULL) {
        return (val);
    }
    
    ixmlSearchNs(node->ownerDocument, node, prefix, namespace);
    if (*namespace ==NULL) {
				       ...
				       const xmlChar **local)
{
    ...
    *local = NULL;
    if (prefix != 0)
	*prefix = NULL;
    ret = ixmlValidateQName(value, 1);
    if (ret > 0) {
	xmlSchemaPSimpleTypeErr(ctxt,
	    XML_SCHEMAP_S4S_ATTR_INVALID_VALUE,
	    ownerDes, ownerItem,  attr,
	    xmlSchemaGetBuiltInType(XML_SCHEMAS_QNAME),
	    "QName", value,
	    NULL, NULL, NULL);
/*	strcpy((char*)*local,(char*)value);*/
          *local=value;
	return (ctxt->err);
    } else if (ret < 0)
	return (-1);
	...
	pNode = &((((IXML_Attr *)attr)->ownerElement)->n);
	
	ixmlSearchNs(attr->ownerDocument, pNode, NULL, uri);
	...
	if (*uri == NULL)
	{
		if (schema->flags & XML_SCHEMAS_INCLUDING_CONVERT_NS)
		{
	    	...
	    	* target namespace.
	    	*/
			*uri = (char*)xmlStrdup(BAD_CAST schema->targetNamespace);
    ...
    */
    *local = ixmlSplitQName2(value, &pref);  /* TO BE FIXED*/

	if (prefix != 0)
	*prefix = pref;
	pNode = &((((IXML_Attr *)attr)->ownerElement)->n);

	ixmlSearchNs(attr->ownerDocument, pNode, pref, uri);
 ...
 */
const xmlChar *
ixmlSplitQName3(const xmlChar *name, int *len) {
   int l = 0;
				       ...
				       const xmlChar **local)
{
   ...
   /* value = xmlSchemaGetNodeContent(ctxt, (xmlNodePtr) attr);*/
    value = ixmlNode_getNodeValueEx(attr);
				   ...
				   const xmlChar **local)
{
    ...
    attr = ixmlSchemaGetPropNode(ownerElem, name);
    if (attr == NULL) {
	*local = NULL;
	*uri = NULL;
	return (0);
 ...
 */
static int
xmlGetMaxOccurs(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node,
		int min, int max, int def, const char *expected)
{
    ...
    attr = ixmlSchemaGetPropNode(node, "maxOccurs");
    if (attr == NULL)
	return (def);
    val = ixmlNode_getNodeValueEx(attr);
 ...
 */
static int
xmlGetMinOccurs(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node,
		int min, int max, int def, const char *expected)
{
    const xmlChar *val, *cur;
    int ret = 0;
    ...
    attr = ixmlSchemaGetPropNode(node, "minOccurs");
    if (attr == NULL)
	return (def);
	val = ixmlNode_getNodeValueEx(attr);
			   ...
			   IXML_Node *node)
{
    ...
    value = ixmlNode_getNodeValueEx(node);
...
xmlSchemaParseAttributeGroup(xmlSchemaParserCtxtPtr ctxt,
                             xmlSchemaPtr schema, IXML_Node *node,
			     int topLevel);
#endif
			   ...
			   xmlSchemaTypePtr type)
{
    ...
    */
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("Internal error: xmlSchemaPvalueAttrNode, the given "
	    "type '%s' is not a built-in type.\n",type->name);
#endif
/*
	xmlSchemaPErr(ctxt, attr,
	    XML_SCHEMAP_INTERNAL,
	...
	case XML_SCHEMAS_NCNAME:
	    /*ret = ixmlValidateNCName(value, 1);*/
	    break;
	case XML_SCHEMAS_QNAME:
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaPvalueAttrNode, use "
			   "the function xmlSchemaExtractSchemaQNamePropvalueidated "
		 	   "for extracting QName values instead.\n");
#endif
/*
	    xmlSchemaPErr(ctxt, attr,
		XML_SCHEMAP_INTERNAL,
...
#ifdef XMLDEBUG
		printf("\nxmlSchemaPValAttrNodeValue: Unsupported XML_SCHEMAS_ANYURI found !\n");
#endif
#if 0
	    if (value != NULL) {
	...
	case XML_SCHEMAS_LANGUAGE:
	   /* if (ixmlCheckLanguageID(value) != 1)
		ret = 1;*/
	    break;
	default: {
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaPvalueAttrNode, "
...
int
ixmlCheckLanguageID(const xmlChar * lang)
{
		       ...
		       const xmlChar **value)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("Internal error: xmlSchemaPValAttr, the given "
	    	   "type '%s' is not a built-in type.\n", type->name);
#endif
...
xmlSchemaParseAttrDecls(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
                        IXML_Node *child, xmlSchemaTypePtr type)
{
    xmlSchemaAttributePtr lastattr, attr;

    lastattr = NULL;
    while ((IS_IXMLSCHEMA(child, "attribute")) ||
           (IS_IXMLSCHEMA(child, "attributeGroup"))) {
        attr = NULL;
        if (IS_IXMLSCHEMA(child, "attribute")) {
            attr = xmlSchemaParseAttribute(ctxt, schema, child, 0);
        } else if (IS_IXMLSCHEMA(child, "attributeGroup")) {
...
#if 0
 ANNOTATION: Will be supported later
                         ...
                         IXML_Node *node)
{
    ...
    */
	...
	if (!pAttribs)
	{
   	...
   	ulAttrCount = ixmlNamedNodeMap_getLength(pAttribs);
	//printf("Got attrib count %d\n", ulAttrCount);

	for (j=0; j<ulAttrCount; j++)
	{
	     	pAttribN = ixmlNamedNodeMap_item(pAttribs,j);

		if (((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "id"))) ||
			((pAttribN->namespaceURI != NULL) &&
			xmlStrEqual(pAttribN->namespaceURI, xmlSchemaNs))) {
    ...
    */
    ...
    pNodeList = ixmlNode_getChildNodes(node);
    n = ixmlNodeList_length(pNodeList);

    for (i=0; i<n; i++)
    {
      child = ixmlNodeList_item(pNodeList, i);

      	if (IS_IXMLSCHEMA(child, "appinfo")) {
	    ...
	    */
		...
		pAttribs = ixmlNode_getAttributes(child);
   		if (!pAttribs)
		{
			return NULL;
		}
	    ...
	    for (j=0; j<ulAttrCount; j++)
	   	{
			...
			if (((pAttribN->namespaceURI == NULL) &&
				(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "source"))) ||
				((pAttribN->namespaceURI  != NULL) &&
				xmlStrEqual(pAttribN->namespaceURI , xmlSchemaNs)))
			{
	 ...
	 } else if (IS_IXMLSCHEMA(child, "documentation")) {
	    ...
	    */
		...
		for (j=0; j<ulAttrCount; j++)
		{
			...
			if (pAttribN->namespaceURI == NULL) {
			    if (!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "source")) {
				xmlSchemaPIllegalAttrErr(ctxt,
				    XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
				    NULL, NULL, pAttribN);
			    }
			} else {
			    if (xmlStrEqual(pAttribN->namespaceURI, xmlSchemaNs) ||
				(xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "lang") &&
				(!xmlStrEqual(pAttribN->namespaceURI, XML_XML_NAMESPACE)))) {
                    ...
                    IXML_Node *node)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaParseFacet:Error allocating facet\n");
#endif
/*        xmlSchemaPErrMemory(ctxt, "allocating facet", node);*/
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaParseFacet:Facet %s has no value\n", node->nodeName);
#endif
/*        xmlSchemaPErr2(ctxt, node, child, XML_SCHEMAP_FACET_NO_VALUE,
    ...
    if (IS_IXMLSCHEMA(node, "minInclusive")) {
        facet->type = XML_SCHEMA_FACET_MININCLUSIVE;
    } else if (IS_IXMLSCHEMA(node, "minExclusive")) {
        facet->type = XML_SCHEMA_FACET_MINEXCLUSIVE;
    } else if (IS_IXMLSCHEMA(node, "maxInclusive")) {
        facet->type = XML_SCHEMA_FACET_MAXINCLUSIVE;
    } else if (IS_IXMLSCHEMA(node, "maxExclusive")) {
        facet->type = XML_SCHEMA_FACET_MAXEXCLUSIVE;
    } else if (IS_IXMLSCHEMA(node, "totalDigits")) {
        facet->type = XML_SCHEMA_FACET_TOTALDIGITS;
    } else if (IS_IXMLSCHEMA(node, "fractionDigits")) {
        facet->type = XML_SCHEMA_FACET_FRACTIONDIGITS;
    } else if (IS_IXMLSCHEMA(node, "pattern")) {
        facet->type = XML_SCHEMA_FACET_PATTERN;
    } else if (IS_IXMLSCHEMA(node, "enumeration")) {
        facet->type = XML_SCHEMA_FACET_ENUMERATION;
    } else if (IS_IXMLSCHEMA(node, "whiteSpace")) {
        facet->type = XML_SCHEMA_FACET_WHITESPACE;
    } else if (IS_IXMLSCHEMA(node, "length")) {
        facet->type = XML_SCHEMA_FACET_LENGTH;
    } else if (IS_IXMLSCHEMA(node, "maxLength")) {
        facet->type = XML_SCHEMA_FACET_MAXLENGTH;
    } else if (IS_IXMLSCHEMA(node, "minLength")) {
        facet->type = XML_SCHEMA_FACET_MINLENGTH;
    } else {
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaParseFacet:Unknown facet type %s\n", node->nodeName);
#endif
/*        xmlSchemaPErr2(ctxt, node, child, XML_SCHEMAP_UNKNOWN_FACET_TYPE,
    ...
    if (IS_IXMLSCHEMA(child, "annotation")) {
		/* ANNOTATION: Will be supported later
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaParseFacet:Facet %s has unexpected child content\n",
                 node->nodeName);
#endif
			 ...
			 IXML_Node *node)
{
     ...
     */
    attr = ixmlSchemaGetPropNode(node, "namespace");
    /*ns = xmlSchemaGetNodeContent(ctxt, (IXML_Node *) attr);*/
    ns = ixmlNode_getNodeValueEx(attr);
	...
	if (maxOccurs < 1) {
	    ...
	    */
	    xmlSchemaPCustomAttrErr(ctxt,
						...
						NULL, item, ixmlSchemaGetPropNode(node, "maxOccurs"),
	    ...
	    */
		...
		NULL, item, ixmlSchemaGetPropNode(node, "minOccurs"),
                  ...
                  IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "any %d", ctxt->counter++ + 1);
    ...
    */
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaParseAny:Sequence %s has unexpected content\n", type->name);
#endif
    ...
    */
	...
	for (j=0; j<ulAttrCount; j++)
	{
		...
		if (((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "id"))) ||
			((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "namespace"))) ||
			((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "processContents")))  ||
			((pAttribN->namespaceURI != NULL) &&
			xmlStrEqual(pAttribN->namespaceURI, xmlSchemaNs))) {
                        ...
                        IXML_Node *node, int topLevel)
{
    ...
    if ((ctxt == NULL) || (schema == NULL) || (node == NULL))
        return (NULL);
    attr = ixmlSchemaGetPropNode(node, "ref");
    nameAttr = ixmlSchemaGetPropNode(node, "name");
	...
	*/
    ...
    if (isRef) {
	char buf[50];
	const xmlChar *refNs = NULL;
	const xmlChar *ref = NULL, *refPrefix = NULL;
	...
	*/
    ...
    for (i=0; i<ulAttrCount; i++)
	{
		attr = ixmlNamedNodeMap_item(pAttribs,i);
	    if (attr->namespaceURI== NULL) {
		if (xmlStrEqual(attr->nodeName, BAD_CAST "type") ||
		    xmlStrEqual(attr->nodeName, BAD_CAST "form")) {
		    /*
		    * 3.2.3 : 3.2
		...
		} else if ((!xmlStrEqual(attr->nodeName, BAD_CAST "ref")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "use")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "id")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "name")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "fixed")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "default"))) {
		    xmlSchemaPIllegalAttrErr(ctxt,
			XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
			&repName, (xmlSchemaTypePtr) ret, attr);
		}
	    } else if (xmlStrEqual(attr->namespaceURI, xmlSchemaNs)) {
		xmlSchemaPIllegalAttrErr(ctxt,
		    XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
		    &repName, (xmlSchemaTypePtr) ret, attr);
	    }
	...
	*/
	if (topLevel) {
	    ...
	    attr = ixmlSchemaGetPropNode(node, "form");
	    if (attr != NULL) {
/*		attrValue = xmlSchemaGetNodeContent(ctxt, (xmlNodePtr) attr); */
		attrValue = ixmlNode_getNodeValueEx(attr);
	...
	*/
    ...
    for (i=0; i<ulAttrCount; i++)
	{
		attr = ixmlNamedNodeMap_item(pAttribs,i);
	    if (attr->namespaceURI == NULL) {
		if ((!xmlStrEqual(attr->nodeName, BAD_CAST "id")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "default")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "fixed")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "name")) &&
		    (!xmlStrEqual(attr->nodeName, BAD_CAST "type"))) {
		    if ((topLevel) ||
		        ((!xmlStrEqual(attr->nodeName, BAD_CAST "form")) &&
			 (!xmlStrEqual(attr->nodeName, BAD_CAST "use")))) {
			xmlSchemaPIllegalAttrErr(ctxt,
    ...
    * Attribute "default".
    */
    attr = ixmlSchemaGetPropNode(node, "default");
    if (attr != NULL) {
	/*
	* 3.2.3 : 1
	...
	*/
	...
	ret->defValue = ixmlNode_getNodeValueEx(attr);
    }
    if (topLevel == 0) {
	/*
	* Attribute "use".
	*/
	attr = ixmlSchemaGetPropNode(node, "use");
	if (attr != NULL) {
	    /*attrValue = xmlSchemaGetNodeContent(ctxt, (xmlNodePtr) attr);*/
		attrValue = ixmlNode_getNodeValueEx(attr);
...
/*    n = ixmlNodeList_length(pNodeList); */
    ...
    child = ixmlNodeList_item(pNodeList, 0);
    ...
    if (isRef) {
	if (child != NULL) {
	    if (IS_IXMLSCHEMA(child, "simpleType"))
		/*
		* 3.2.3 : 3.2
		* If ref is present, then all of <simpleType>,
		* form and type must be absent.
			     ...
			     int topLevel)
{
	...
	*/
	...
	name = ixmlNode_getNodeValueEx(nameAttr);
	/*
	...
	*/
	...
	xmlSchemaPValAttrNodeQName(ctxt, schema,
        ...
        T_snprintf(buf, 49, "#aGrRef %d", ctxt->counter++ + 1);
	name = (const xmlChar *) buf;
	if (name == NULL) {
	...
	for (j=0; j<ulAttrCount; j++)
	{
		...
		if ((topLevel == 0)|| ((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "ref"))) ||
			((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "name"))) ||
			((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(ixmlNode_getNodeName(pAttribN), BAD_CAST "id")))  ||
			((pAttribN->namespaceURI != NULL) &&
			xmlStrEqual(pAttribN->namespaceURI, xmlSchemaNs))) {
    ...
    */
	...
	if (IS_IXMLSCHEMA(child, "anyAttribute")) {
                      ...
                      IXML_Node *node, int topLevel)
{
    ...
    IXML_NamedNodeMap *pAttribs = NULL;
    int j = 0, ulAttrCount = 0;
    char pAttribName[XML_MAX_NAMELEN];
	...
	/*name = xmlSchemaGetNodeContent(ctxt, (IXML_Node *) nameAttr);*/
	name = ixmlNode_getNodeValueEx(nameAttr);
	...
	*/
	...
	xmlSchemaPValAttrNodeQName(ctxt, schema,
        ...
        T_snprintf(buf, 49, "#eRef %d", ctxt->counter++ + 1);
	name = (const xmlChar *) buf;
...
/*	    if (repName != NULL)
	...
	*/
	...
	for (j=0; j<ulAttrCount; j++)
	{
		...
		if (pAttribN->namespaceURI == NULL)
		{
			if (xmlStrEqual(pAttribN->nodeName, BAD_CAST "ref") ||
		  	  xmlStrEqual(pAttribN->nodeName, BAD_CAST "name") ||
		   	 xmlStrEqual(pAttribN->nodeName, BAD_CAST "id") ||
		 	   xmlStrEqual(pAttribN->nodeName, BAD_CAST "maxOccurs") ||
		   	 xmlStrEqual(pAttribN->nodeName, BAD_CAST "minOccurs"))
			{
		    		continue;
			}
			else
			{
				...
				&repName, (xmlSchemaTypePtr) ret, pAttribN,
				"Only the attributes 'minOccurs', 'maxOccurs' and "
				"id' are allowed in addition to 'ref'");
		   		 break;
			}
	      }
	      else if (xmlStrEqual(pAttribN->namespaceURI, xmlSchemaNs))
	     {
			xmlSchemaPIllegalAttrErr(ctxt,
		    XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
		    &repName, (xmlSchemaTypePtr) ret, pAttribN);
	...
	*/
	if (topLevel) {
	    ...
	    attr = ixmlSchemaGetPropNode(node, "form");
	    if (attr != NULL) {
		/*attrValue = xmlSchemaGetNodeContent(ctxt, attr);*/
		attrValue = ixmlNode_getNodeValueEx(attr);
		if (xmlStrEqual(attrValue, BAD_CAST "qualified")) {
		    ns = schema->targetNamespace;
		} else if (!xmlStrEqual(attrValue, BAD_CAST "unqualified")) {
	...
	{
	     	pAttribN = ixmlNamedNodeMap_item(pAttribs,j);
	     	strcpy(pAttribName, ixmlNode_getNodeName(pAttribN));

		if (pAttribN->namespaceURI == NULL)
		{
			if ((!xmlStrEqual(pAttribName, BAD_CAST "name"))
			 	&&
				(!xmlStrEqual(pAttribName, BAD_CAST "type"))
			 	&&
				(!xmlStrEqual(pAttribName, BAD_CAST "id"))
			 	&&
				(!xmlStrEqual(pAttribName, BAD_CAST "default"))
			 	&&
				(!xmlStrEqual(pAttribName, BAD_CAST "fixed"))
			 	&&
				(!xmlStrEqual(pAttribName, BAD_CAST "block"))
			 	&&
				(!xmlStrEqual(pAttribName, BAD_CAST "nillable")))
			{
				if (topLevel == 0 )
				{
					if ((!xmlStrEqual(pAttribName, BAD_CAST "maxOccurs")) &&
					    (!xmlStrEqual(pAttribName, BAD_CAST "minOccurs")) &&
			   		     (!xmlStrEqual(pAttribName, BAD_CAST "form")))
					{
						if (xmlStrEqual(pAttribName, BAD_CAST "substitutionGroup"))
						{
			      ...
			      } else if ((!xmlStrEqual(pAttribName, BAD_CAST "final")) &&
				(!xmlStrEqual(pAttribName, BAD_CAST "abstract")) &&
				(!xmlStrEqual(pAttribName, BAD_CAST "substitutionGroup")))
				{
					xmlSchemaPIllegalAttrErr(ctxt,
					    XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
				    &repName, (xmlSchemaTypePtr) ret, pAttribN);
	...
	*/
	if (topLevel) {
	    ...
	    * Attribute "final".
	    */
	    attr = ixmlSchemaGetPropNode(node, "final");
	    if (attr == NULL) {
	...
	* Attribute "block".
	*/
	attr = ixmlSchemaGetPropNode(node, "block");
	if (attr == NULL) {
	    ret->flags |= XML_SCHEMAS_ELEM_BLOCK_ABSENT;
	} else {
	   /* attrValue = xmlSchemaGetNodeContent(ctxt, (IXML_Node *) attr); */
	   attrValue = ixmlNode_getNodeValueEx(attr);
	    if (xmlSchemaPValAttrBlockFinal(attrValue, &(ret->flags),
		-1,
	...
	attr = ixmlSchemaGetPropNode(node, "fixed");
	if (attr != NULL) {
	    /*fixed = xmlSchemaGetNodeContent(ctxt, (IXML_Node *) attr);*/
	    fixed = ixmlNode_getNodeValueEx(attr);
	    if (ret->value != NULL) {
		/*
		* 3.3.3 : 1
		* default and fixed must not both be present.
    ...
    */
	...
	if (IS_IXMLSCHEMA(child, "complexType")) {
	    /*
	    * 3.3.3 : 3
	    ...
	    */
	    ...
	    child = child->nextSibling;
	} else if (IS_IXMLSCHEMA(child, "simpleType")) {
	    /*
	    * 3.3.3 : 3
	...
	while ((IS_IXMLSCHEMA(child, "unique")) ||
	    (IS_IXMLSCHEMA(child, "key")) || (IS_IXMLSCHEMA(child, "keyref"))) {
	    TODO child = child->nextSibling;
	}
	if (child != NULL) {
	    xmlSchemaPContentErr(ctxt,
		XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED,
                    ...
                    IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "#union %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
    ...
    */
    ...
    for (j=0; j<ulAttrCount; j++)
    {
	   attr = ixmlNamedNodeMap_item(pAttribs,j);
	   /*pAttribName = ixmlNode_getNodeName(attr);*/
	   strcpy(pAttribName , ixmlNode_getNodeName(attr));

	   if (attr->namespaceURI == NULL) {
	   	    if ((!xmlStrEqual(pAttribName, BAD_CAST "id")) &&
	   		(!xmlStrEqual(pAttribName, BAD_CAST "memberTypes"))) {
	   		xmlSchemaPIllegalAttrErr(ctxt,
	   		    XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
	   		    NULL, type, attr);
	   	    }
    ...
    */
    ...
    while (IS_IXMLSCHEMA(child, "simpleType")) {
                   ...
                   IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "#list %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
    type->node = node;
    ...
    */
	...
	for (j=0; j<ulAttrCount; j++)
	{
	   ...
	   if (attr->namespaceURI == NULL) {
		    if ((!xmlStrEqual(pAttribName, BAD_CAST "id")) &&
			(!xmlStrEqual(pAttribName, BAD_CAST "itemType"))) {
			xmlSchemaPIllegalAttrErr(ctxt,
			    XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
			    NULL, type, attr);
		    }
                         ...
                         IXML_Node *node, int topLevel)
{
	...
	*/
        T_snprintf(buf, 39, "#ST %d", ctxt->counter++ + 1);
	...
	*/
	...
	for (j=0; j<ulAttrCount; j++)
	{
	   ...
	   if (attr->namespaceURI == NULL) {
		if ((!xmlStrEqual(pAttribName, BAD_CAST "id")) &&
		    (!xmlStrEqual(pAttribName, BAD_CAST "name")) &&
		    (!xmlStrEqual(pAttribName, BAD_CAST "final"))) {
		    xmlSchemaPIllegalAttrErr(ctxt,
			XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
			&des, type, attr);
		}
    ...
    */
    ...
    subtype = NULL;
    if (IS_IXMLSCHEMA(child, "restriction")) {
        subtype = (xmlSchemaTypePtr)
            xmlSchemaParseRestriction(ctxt, schema, child);
	     child = child->nextSibling;
    } else if (IS_IXMLSCHEMA(child, "list")) {
        subtype = (xmlSchemaTypePtr)
            xmlSchemaParseList(ctxt, schema, child);
            child = child->nextSibling;
    } 
    /*else if (IS_IXMLSCHEMA(child, "union")) {
        subtype = (xmlSchemaTypePtr)
                    ...
                    IXML_Node *node, int topLevel)
{
    ...
    */
    ...
    if (name == NULL) {
		...
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Group definition or particle: One of the attributes \"name\" "
...
/*            xmlSchemaPErr2(ctxt, node, child,
	...
	if (refNs == NULL)
        ...
        T_snprintf(buf, 99, "anongroup %d", ctxt->counter++ + 1);
        name = (const xmlChar *) buf;
    }
    ...
    subtype = NULL;
    if (IS_IXMLSCHEMA(child, "all")) {
        subtype = (xmlSchemaTypePtr)
            xmlSchemaParseAll(ctxt, schema, child);
        child = child->nextSibling;
    } else if (IS_IXMLSCHEMA(child, "choice")) {
        subtype = xmlSchemaParseChoice(ctxt, schema, child);
        child = child->nextSibling;
    } else if (IS_IXMLSCHEMA(child, "sequence")) {
        subtype = (xmlSchemaTypePtr)
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("Group definition \"%s\" has unexpected content.\n", type->name);
#endif
                  ...
                  IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "all%d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
    type->node = node;
    ...
    for (i=0; i<n; i++)
    {
         ...
         while (IS_IXMLSCHEMA(child, "element")) {
             subtype = (xmlSchemaTypePtr)
       	    ...
       	    if (subtype->minOccurs > 1)
			{
				xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
				printf("invalid value for minOccurs (must be 0 or 1).\n");
#endif
       	    ...
       	    if (subtype->maxOccurs > 1)
			{
				xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
				printf("invalid value for maxOccurs (must be 0 or 1).\n");
#endif
		...
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("xmlSchemaParseAll:<all> has unexpected content.\n");
#endif
 ...
 */
static void
xmlSchemaCleanupDoc(xmlSchemaParserCtxtPtr ctxt, IXML_Node *root)
{
   ...
   IXML_Node *cur;
    ...
    while (cur != NULL) {
        if (delete != NULL) {
            xmlUnlinkNode(delete); /*TO BE  FIXED*/
            xmlFreeNode(delete);  /*TO BE FIXED*/
            delete = NULL;
        }
        if (cur->nodeType == XML_TEXT_NODE) {
            if (IS_BLANK_NODE(cur)) {
                if (xmlNodeGetSpacePreserve(cur) != 1) {  /*TO BE FIXED*/
                    delete = cur;
                }
            }
        ...
        if (cur->firstChild != NULL) {
            if ((cur->firstChild->nodeType != XML_ENTITY_DECL) &&
                (cur->firstChild ->nodeType != XML_ENTITY_REF_NODE) &&
                (cur->firstChild->nodeType != XML_ENTITY_NODE)) {
                cur = cur->firstChild;
                continue;
            }
        }
...
static void
ixmlSchemaParseSchemaDefaults(xmlSchemaParserCtxtPtr ctxt,
			     		xmlSchemaPtr schema,
			     		IXML_Node *node)
{
    IXML_Node *attr = NULL;
    ...
    pAttribN = ixmlSchemaGetPropNode(node, "elementFormDefault");

    if (pAttribN != NULL) {
    	val = ixmlNode_getNodeValueEx(pAttribN);
    ...
    pAttribN = ixmlSchemaGetPropNode(node, "attributeFormDefault");
    ...
    pAttribN = ixmlSchemaGetPropNode(node, "finalDefault");
    ...
    pAttribN = ixmlSchemaGetPropNode(node, "blockDefault");
			     ...
			     IXML_Node *node)
{
    ...
    attr = ixmlSchemaGetPropNode(node, "elementFormDefault");
    if (attr != NULL) {
	/*val = xmlSchemaGetNodeContent(ctxt, attr);*/
	val = ixmlNode_getNodeValueEx(attr);
	if (xmlSchemaPValAttrFormDefault(val, &schema->flags,
	    XML_SCHEMAS_QUALIF_ELEM) != 0) {
	    xmlSchemaPSimpleTypeErr(ctxt,
    ...
    attr = ixmlSchemaGetPropNode(node, "attributeFormDefault");
    if (attr != NULL) {
	/*val = xmlSchemaGetNodeContent(ctxt, attr);*/
	val = ixmlNode_getNodeValueEx(attr);
	if (xmlSchemaPValAttrFormDefault(val, &schema->flags,
    ...
    attr = ixmlSchemaGetPropNode(node, "finalDefault");
    if (attr != NULL) {
	/*val = xmlSchemaGetNodeContent(ctxt, attr);*/
	val = ixmlNode_getNodeValueEx(attr);
	if (xmlSchemaPValAttrBlockFinal(val, &(schema->flags), -1,
    ...
    attr = ixmlSchemaGetPropNode(node, "blockDefault");
    if (attr != NULL) {
	/*val = xmlSchemaGetNodeContent(ctxt, attr);*/
	val = ixmlNode_getNodeValueEx(attr);
	if (xmlSchemaPValAttrBlockFinal(val, &(schema->flags), -1,
...
static void
ixmlSchemaParseSchemaTopLevel(xmlSchemaParserCtxtPtr ctxt,
                             xmlSchemaPtr schema, IXML_Node *nodes)
{
    IXML_Node *child;
    IXML_NodeList *pNodeList;
    /*xmlSchemaAnnotPtr annot;*/
    ...
    pNodeList = ixmlNode_getChildNodes(nodes);
    n = ixmlNodeList_length(pNodeList);
    ...
    for (i=0; i<n; i++)
    {
	    ...
	    } else if (IS_IXMLSCHEMA(child, "import")) {
		/*
		 *  IMPORT: Will be supported later
		 */
	   	/*xmlSchemaParseImport(ctxt, schema, child);*/
	   	TODO
	    } else if (IS_IXMLSCHEMA(child, "include")) {
		/*
		 *  INCLUDE: Will be supported later
		 */
	    ...
	    /*else if (IS_IXMLSCHEMA(child, "redefine")) {
	  	TODO
	    }*/
/*
	    else
...
#if 0
	while (IS_IXMLSCHEMA(child, "annotation")) {
	    /* ANNOTATION: Will be supported later
		annot = xmlSchemaParseAnnotation(ctxt, schema, child);
	    if (schema->annot == NULL)
		schema->annot = annot;
			  ...
			  int absolute)
{
    ...
    */
	...
	base = xmlNodeGetBase(node->doc, node); /* TO BE FIXED */
	if (base == NULL) {
	    URI = xmlBuildURI(location, node->doc->URL); /* TO BE FIXED */
	} else {
	    URI = xmlBuildURI(location, base); /* TO BE FIXED */
	    xmlFree(base);
	}
	if (URI != NULL) {
	    location = xmlDictLookup(ctxt->dict, URI, -1); /* TO BE FIXED */
	    xmlFree(URI);
	}
    }
	...
	*doc = ixmlLoadDocument((char*)location);
                     ...
                     IXML_Node *node)
{
    ...
    const xmlChar *targetNamespace = NULL, *oldTNS, *url;
    IXML_NamedNodeMap *attr;
    IXML_Document *doc;
    IXML_Node *root;
    int flags = 0;
    int i, n, ulAttrCount = 0;
    IXML_Node *pAttribN = NULL;
    char pAttribName[XML_MAX_NAMELEN];
    IXML_NodeList *pNodeList;
    ...
    */
//    attr = node->properties;
    attr = ixmlNode_getAttributes(node);
	if (!attr)
	{
		return (-1);
	}

    ulAttrCount = ixmlNamedNodeMap_getLength(attr);
    //printf("Got attrib count %d\n", ulAttrCount);

   for (i=0; i<ulAttrCount; i++)
   {
   	pAttribN = ixmlNamedNodeMap_item(attr,i);
   	strcpy(pAttribName, ixmlNode_getNodeName(pAttribN));

	if (pAttribN->namespaceURI == NULL) {
		if ((!xmlStrEqual(pAttribName, BAD_CAST "id")) &&
		(!xmlStrEqual(pAttribName, BAD_CAST "namespace")) &&
		(!xmlStrEqual(pAttribName, BAD_CAST "schemaLocation"))) {
		xmlSchemaPIllegalAttrErr(ctxt,
		    XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
		    NULL, NULL, pAttribN);
	    }
...
/*	attr = attr->next; */
 ...
 /*   child = node->children; */
   ...
   /* ret = xmlSchemaAcquireSchemaDoc(ctxt, schema, node, namespace,
	...
	/*acquiring the schema from namespace*/
	printf("\n ACQUIRING SCHEMA NAMESPACE = %s\n", namespace);
	printf("\n schemaLocation = %s\n", schemaLocation);

	doc = ixmlLoadDocument((char*)schemaLocation);
	...
	*/
	...
	/*ctxt->URL = doc->URL;*/
	strcpy((char*)ctxt->URL, (char*)schemaLocation);
	flags = schema->flags;
...
/*	root = xmlDocGetRootElement(doc); */
	...
	ixmlSchemaParseSchemaDefaults(ctxt, schema, root);
	schema->targetNamespace = targetNamespace;
	ixmlSchemaParseSchemaTopLevel(ctxt, schema, root);
	/*
                      ...
                      IXML_Node *node)
{
 ...
 /*   xmlAttrPtr attr; */
    ...
    */
     ...
     */
    attr = ixmlSchemaGetPropNode(node, "schemaLocation");
    if (attr != NULL) {
    if (xmlSchemaPValAttrNode(ctxt, NULL, NULL, attr,
	    xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI), &schemaLocation) != 0)
	    return (-1);
#if 0
	base = xmlNodeGetBase(node->ownerDocument, node);/*TO BE FIXED*/
	if (base == NULL) {
	    uri = xmlBuildURI(schemaLocation, node->ownerDocument->URL);/*TO BE FIXED*/
	} else {
	    uri = xmlBuildURI(schemaLocation, base); /* TO BE FIXED */
	    xmlFree(base);
	}
	if (uri != NULL) {
    ...
    */
	...
	include->schemaLocation = (char*)malloc(strlen(schemaLocation));

	if (include->schemaLocation)
	{
		strcpy((char*)include->schemaLocation, (char*)schemaLocation);
	}
	else
	{
		return (-1);
                     ...
                     IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "choice %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
    type->node = node;
    ...
    */
	...
	for (j=0; j<ulAttrCount; j++)
	{
		...
		if (((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(pAttribName, BAD_CAST "id"))) ||
			((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(pAttribName, BAD_CAST "maxOccurs"))) ||
			((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(pAttribName, BAD_CAST "minOccurs")))  ||
			((pAttribN->namespaceURI != NULL) &&
			xmlStrEqual(pAttribN->namespaceURI, xmlSchemaNs))) {
...
*/
    ...
    */
    ...
    while ((IS_IXMLSCHEMA(child, "element")) ||
           (IS_IXMLSCHEMA(child, "group")) ||
           (IS_IXMLSCHEMA(child, "any")) ||
           (IS_IXMLSCHEMA(child, "choice")) ||
           (IS_IXMLSCHEMA(child, "sequence"))) {
        subtype = NULL;
        if (IS_IXMLSCHEMA(child, "element")) {
            subtype = (xmlSchemaTypePtr)
                xmlSchemaParseElement(ctxt, schema, child, 0);
        } else if (IS_IXMLSCHEMA(child, "group")) {
        /*    subtype = xmlSchemaParseGroup(ctxt, schema, child, 0);*/
        } else if (IS_IXMLSCHEMA(child, "any")) {
                       ...
                       IXML_Node *node)
{
    ...
    oldcontainer = ctxt->container;
    T_snprintf((char *) name, 30, "#seq %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
    type->node = node;
                          ...
                          IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "#restr %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
    ...
    */
	...
	for (j=0; j<ulAttrCount; j++)
	{
		...
		if (pAttribN->namespaceURI == NULL) {
			 if ((!xmlStrEqual(pAttribName, BAD_CAST "id")) &&
				(!xmlStrEqual(pAttribName, BAD_CAST "base"))) {
				xmlSchemaPIllegalAttrErr(ctxt,
				 XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
										    NULL, type, pAttribN);
			}
    ...
    */
    ...
    ctxt->container = name;
        ...
        if ((ctxt->ctxtType->type == XML_SCHEMA_TYPE_SIMPLE) ||
       	...
       	* *Single Facet Value*
       	*/
       	while ((IS_IXMLSCHEMA(child, "minInclusive")) ||
       	    (IS_IXMLSCHEMA(child, "minExclusive")) ||
       	    (IS_IXMLSCHEMA(child, "maxInclusive")) ||
       	    (IS_IXMLSCHEMA(child, "maxExclusive")) ||
       	    (IS_IXMLSCHEMA(child, "totalDigits")) ||
       	    (IS_IXMLSCHEMA(child, "fractionDigits")) ||
       	    (IS_IXMLSCHEMA(child, "pattern")) ||
       	    (IS_IXMLSCHEMA(child, "enumeration")) ||
       	    (IS_IXMLSCHEMA(child, "whiteSpace")) ||
       	    (IS_IXMLSCHEMA(child, "length")) ||
       	    (IS_IXMLSCHEMA(child, "maxLength")) ||
       	    (IS_IXMLSCHEMA(child, "minLength"))) {
       	...
       	*/
       	if (lastfacet != NULL) {
       		...
       		if (facetLink == NULL) 
       		{
				xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
				printf("xmlSchemaParseRestriction:Error allocating a facet link\n");
#endif
                       ...
                       IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "extension %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
                            ...
                            xmlSchemaPtr schema, IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "simpleContent %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    ...
    if (IS_IXMLSCHEMA(child, "restriction")) {
        subtype = (xmlSchemaTypePtr)
            xmlSchemaParseRestriction(ctxt, schema, child);
        child = child->nextSibling;
    } else if (IS_IXMLSCHEMA(child, "extension")) {
        subtype = (xmlSchemaTypePtr)
                             ...
                             xmlSchemaPtr schema, IXML_Node *node)
{
    ...
    T_snprintf((char *) name, 30, "#CC %d", ctxt->counter++ + 1);
    type = xmlSchemaAddType(ctxt, schema, name, NULL, node);
    if (type == NULL)
        return (NULL);
    ...
    */
	...
	for (j=0; j<ulAttrCount; j++)
	{
		...
		if (((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(pAttribName, BAD_CAST "id"))) ||
			((pAttribN->namespaceURI == NULL) &&
			(!xmlStrEqual(pAttribName, BAD_CAST "mixed"))) ||
			((pAttribN->namespaceURI != NULL) &&
			xmlStrEqual(pAttribN->namespaceURI, xmlSchemaNs))) {
			xmlSchemaPIllegalAttrErr(ctxt,
		XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED,
                          ...
                          IXML_Node *node, int topLevel)
{
	...
	*/
        T_snprintf(buf, 39, "#CT %d", ctxt->counter++ + 1);
	type = xmlSchemaAddType(ctxt, schema, (const xmlChar *)buf, NULL, node);
	if (type == NULL)
	    return (NULL);
	name = (const xmlChar *) buf;
    ...
    */
    ...
    for (i=0; i<ulAttrCount; i++)
    {
    	attr = ixmlNamedNodeMap_item(pAttribs,i);
		strcpy(pAttribName, ixmlNode_getNodeName(attr));
		/*pAttribValue = ixmlNode_getNodeValueEx(attr);*/
		...
		*/
		type->id = xmlSchemaGetProp(ctxt, node, "id");
	    } else if (xmlStrEqual(pAttribName, BAD_CAST "mixed")) {
		/*
		* Attribute "mixed".
		*/
		if (xmlSchemaPGetBoolNodeValue(ctxt, &des, type, attr))
		...
		*/
		if (xmlStrEqual(pAttribName, BAD_CAST "name")) {
		    /* Pass. */
		} else if (xmlStrEqual(pAttribName, BAD_CAST "abstract")) {
		    /*
		    * Attribute "abstract".
		    */
		    if (xmlSchemaPGetBoolNodeValue(ctxt, &des, type, attr))
		...
		} else if (xmlStrEqual(pAttribName, BAD_CAST "block")) {
		    /*
		    * Attribute "block".
		    */
		    attrValue = ixmlNode_getNodeValueEx(attr);
    ...
    */
    ...
    ctxt->ctxtType = type;
    if (IS_IXMLSCHEMA(child, "simpleContent")) {
#if 0
	/*
	* 3.4.3 : 2.2
	...
	*/
    ...
    else if (IS_IXMLSCHEMA(child, "complexContent")) {
 ...
 */
static xmlSchemaPtr
ixmlSchemaParseSchema(xmlSchemaParserCtxtPtr ctxt, IXML_Node *node)
{
    ...
    */
    ...
    /* if (IS_IXMLSCHEMA(node, "schema")) {
	    printf("\n***** YES IT IS A SCHEMA ......\n");
	    schema = xmlSchemaNewSchema(ctxt);
	    if (schema == NULL)
	    	return (NULL);
    }
   ixmlSchemaParseSchemaDefaults(ctxt, schema, node);
    ixmlSchemaParseSchemaTopLevel(ctxt, schema, node);*/
	...
	*/
	attr = ixmlSchemaGetPropNode(node, "targetNamespace");
	if (attr != NULL) {
	    xmlSchemaPValAttrNode(ctxt, NULL, NULL, attr,
	    ...
	    */
	    ...
	    if(val) {
		schema->targetNamespace = (char *) malloc(strlen(val));
		if(schema->targetNamespace)
	    		strcpy((char *)schema->targetNamespace, (char *) val);
		else
			return (NULL);
	    }
	...
	*/
	...
	printf("\n**** CALLING ixmlSchemaParseSchemaDefaults ...\n");
	ixmlSchemaParseSchemaDefaults(ctxt, schema, node);
	printf("\n****ctxt->nberrors = %d  \n", ctxt->nberrors);
	
	printf("\n**** CALLING xmlSchemaParseSchemaTopLevel ...\n");
        ixmlSchemaParseSchemaTopLevel(ctxt, schema, node);
	printf("\n****ctxt->nberrors = %d  \n", ctxt->nberrors);
    } else {
        IXML_Document *doc;
	...
	doc = node->ownerDocument;
		...
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("The file (null) is not a XML schema.\n");
#endif
	    /*xmlSchemaPErr(ctxt, (IXML_Node*) doc,
		      XML_SCHEMAP_NOT_SCHEMA,
		      "The file \"%s\" is not a XML schema.\n", NULL, NULL);*/
	} else {
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("The file is not a XML schema.\n");
#endif
	    /*xmlSchemaPErr(ctxt, (IXML_Node*) doc,
		      XML_SCHEMAP_NOT_SCHEMA,
...
xmlSchemaNewParserCtxt(const char *URL)
{
    ...
    if (ret == NULL) {
#ifdef XMLDEBUG
	printf("xmlSchemaNewParserCtxt:Error allocating schema parser context\n");
#endif
/*        xmlSchemaPErrMemory(NULL, "allocating schema parser context",
   ...
   /* ret->dict = xmlDictCreate();
	...
	ret->URL = (char*)malloc(strlen(URL));
	if (ret->URL == NULL)
		{
		free(ret);
		return NULL;
		}
    strcpy((char*)ret->URL, (char*)URL);
    ret->includes = 0;
    return (ret);
}
...
xmlSchemaNewParserCtxtUseDict(const char *URL)
{
    xmlSchemaParserCtxtPtr ret;
    /*
    if (URL == NULL)
...
xmlSchemaNewMemParserCtxt(const char *buffer, int size)
{
    ...
    if (ret == NULL) {
#ifdef XMLDEBUG
	printf("xmlSchemaNewMemParserCtxt: Error allocating schema parser context\n");
#endif
/*        xmlSchemaPErrMemory(NULL, "allocating schema parser context",
                            NULL);*/
        return (NULL);
                            ...
                            const xmlChar * name)
{
	...
	case XML_SCHEMA_TYPE_ANY: {
		...
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaBuildAContentModel, "
                   ...
                   xmlSchemaParserCtxtPtr ctxt)
{
    ...
    */
	...
	attr = ixmlSchemaGetPropNode(type->node, "memberTypes");
	cur = type->base;
	do {
	    while (IS_BLANK_CH(*cur))
		cur++;
...
/*	    if (xmlSchemaBuildAttributeUsesOwned(ctxt,
		...
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("xmlSchemaBuildAttributeUsesOwned: Error building attribute uses\n");
#endif
...
xmlSchemaBuildAttributeValidation(xmlSchemaParserCtxtPtr ctxt, xmlSchemaTypePtr type)
{
    ...
    if (type->attributeUses != NULL) {
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaBuildAttributeValidation: "
...
/*        xmlSchemaPErr(ctxt, type->node, XML_SCHEMAP_INTERNAL,
	...
	if (baseType != NULL) {
		...
		if (tmp == NULL) {
			xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
			printf("xmlSchemaBuildAttributeValidation:Error building "
				   "attribute uses of complexType\n");
#endif
		    /*xmlSchemaPErrMemory(ctxt,
			     ...
			     xmlSchemaTypePtr type)
{
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("xmlSchemaCheckDerivationValidSimpleRestriction: The given "
...
/*	xmlSchemaPErr(ctxt, type->node,
	...
	*/
	if (type->facets != NULL) {
		...
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("xmlSchemaCheckDerivationValidSimpleRestriction: failed "
	...
	*/
	...
	xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
	printf("Internal error: xmlSchemaCheckDerivationValidSimpleRestriction: "
	...
	if (type->baseType == xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYSIMPLETYPE)) {
	    ...
	    */
		...
		if (baseItemType == NULL) {
			xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
			printf("xmlSchemaCheckDerivationValidSimpleRestriction: "
			    ...
			    xmlSchemaTypePtr type)
{
    ...
    */
    if (type->subtypes == NULL) {
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaCheckSRCSimpleType, "
...
xmlSchemaCreateVCtxtOnPCtxt(xmlSchemaParserCtxtPtr ctxt)
{
   if (ctxt->vctxt == NULL) {
	...
	if (ctxt->vctxt == NULL) {
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaCreatePCtxtOnVCtxt, "
			      ...
			      IXML_Node *node)
{
    ...
    */
    if ((pctxt == NULL) || (vctxt == NULL)) {
		xmlSchemaErr(pctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaCheckCOSValidDefault, "
    ...
    if (item->contentType == XML_SCHEMA_CONTENT_UNKNOWN) {
        switch (item->type) {
			...
			base = item->baseType;
				...
				XML_SCHEMAP_SRC_RESOLVE,
				NULL, NULL,
				 ixmlSchemaGetPropNode(item->node, "base"),
				...
				for (; cur != NULL; cur = cur->next) {
				    ...
				    */
					...
					if (facet == NULL) {
						xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
						printf("xmlSchemaTypeFixup:Error fixing simpleType\n");
#endif
					    /*xmlSchemaPErrMemory(ctxt,
...
#ifdef DEBUG_TYPE
    if (item->node != NULL) {
	printf("\nxmlSchemaTypeFixup: Type of: %s:%s:\n", name,
		item->node->ownerDocument->n.nodeName);
		...
		*/
		if (typeDecl->type != XML_SCHEMA_TYPE_BASIC) {
		    ...
		    if (base == NULL) {
				xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
				printf("Internal error: xmlSchemaCheckFacet, "
...
/*			xmlSchemaPErr(ctxt, typeDecl->node,
		...
		*/
		...
		vctxt->value = NULL;
				...
				xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
				printf("Type definition '%s': The value '%s' of the "
			    ...
			    const xmlChar * name ATTRIBUTE_UNUSED)
{
    ...
    */
	...
	if (item->subtypes == NULL) {
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaCheckAttrValConstr, "
	...
	*/
	...
	if (item->flags && XML_SCHEMAS_ATTR_FIXED)
	    node = ixmlNode_getAttributeByName(item->node, (DOMString) "fixed");
	else
	    node = ixmlNode_getAttributeByName(item->node, (DOMString) "default");
			    ...
			    const xmlChar * name ATTRIBUTE_UNUSED)
{
	...
	*/
	if (decl->subtypes == NULL) {
		xmlSchemaErr(ctxt);
#ifdef XMLDEBUG
		printf("Internal error: xmlSchemaCheckElemValConstr, "
		"type is missing... skipping validation of "
	...
	*/
	...
	if (decl->node != NULL) {
	    if (decl->flags && XML_SCHEMAS_ELEM_FIXED)
			node = ixmlNode_getAttributeByName(decl->node, (DOMString) "fixed");
	    else
			node = ixmlNode_getAttributeByName(decl->node, (DOMString) "default");
	}
	ctxt->vctxt->node = node;
	ctxt->vctxt->cur = NULL;
...
xmlSchemaParse(xmlSchemaParserCtxtPtr ctxt)
{
    ...
    */
    ...
    if (ctxt->URL != NULL) {
	  xmlDoc = ixmlLoadDocument((char *)ctxt->URL);
	  ...
	  printf("\n**** xmlDoc->n.nodeName = %s \n", xmlDoc->n.nodeName);
	  printf("\n ::xmlDoc->n.nodeName=%s\n",xmlDoc->n.nodeName);
	  printf("\n ::xmlDoc->n.nodeValue= %s\n",xmlDoc->n.nodeValue);
	  printf("\n ::xmlDoc->n.namespaceURI= %s\n",xmlDoc->n.namespaceURI);
	  printf("\n ::xmlDoc->n.prefix= %s\n",xmlDoc->n.prefix);
	  printf("\n ::xmlDoc->n.localName= %s\n",xmlDoc->n.localName);
	  printf("\n ::xmlDoc->n.firstChild.n.nodeName= %s\n",xmlDoc->n.firstChild->nodeName);
	  printf("\n ::xmlDoc->n.firstChild->firstChild->nodeName= %s\n",xmlDoc->n.firstChild->firstChild->nodeName);
	  printf("\n ::xmlDoc->n.firstChild->firstChild->firstAttr->nodeName= %s\n",xmlDoc->n.firstChild->firstChild->firstAttr->nodeName);
	  printf("\n ::xmlDoc->n.firstChild->firstChild->firstAttr->nodeValue= %s\n",xmlDoc->n.firstChild->firstChild->firstAttr->nodeValue);
	  /*printf("\n ::xmlDoc->n.firstChild->firstChild->firstAttr->nextSibling->nodeName= %s\n",xmlDoc->n.firstChild->firstChild->firstAttr->nextSibling->nodeName);
	  printf("\n ::xmlDoc->n.firstChild->firstChild->firstAttr->nextSibling->nodeValue= %s\n",xmlDoc->n.firstChild->firstChild->firstAttr->nextSibling->nodeValue); */
#endif
    }
    else {
    	preserve = 1;
   ...
   if(!xmlDoc)
     ...
     */
    ret = NULL;
    ret = ixmlSchemaParseSchema(ctxt, rootNode);

#ifdef XMLDEBUG
	printf("\n*** AFTER ixmlSchemaParseSchema :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
...
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan 1 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
#if 0
    /*
	...
	printf("\n*** AFTER xmlHashScan 2 :: ctxt->nberrors = %d \n", ctxt->nberrors);
    ...
    */
	...
	printf("\n*** AFTER xmlHashScan 3 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
    /*
...
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan 4 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
...
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan 5 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
...
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan FULL :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
    ...
    */
...
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan 6 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
...
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan 7 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
    ...
    xmlHashScan(ret->typeDecl, (xmlHashScanner) xmlSchemaCheckDefaults,
                ctxt);
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan 8 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
...
#ifdef XMLDEBUG
	printf("\n*** AFTER xmlHashScan 9 :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
...
#ifdef XMLDEBUG
	printf("\n*** AFTER ALL xmlHashScans :: ctxt->nberrors = %d \n", ctxt->nberrors);
#endif
 ...
 */
static int
xmlSchemaRegisterAttributes(xmlSchemaValidCtxtPtr ctxt, IXML_Node *attrs)
{
    xmlSchemaAttrStatePtr tmp;
    ...
    ctxt->attrTop = NULL;
    while (attrs != NULL) {
        if ((attrs->namespaceURI != NULL) && 
            (xmlStrEqual(attrs->namespaceURI, xmlSchemaInstanceNs))) {
            attrs = attrs->nextSibling;
            continue;
        }

		if ((!strncmp(attrs->nodeName,"xmlns:",strlen("xmlns:"))) &&
			(!strcmp(attrs->nodeValue, xmlSchemaInstanceNs)))
		{
			attrs = attrs->nextSibling;
			continue;
		}
			    ...
			    const xmlChar *location)
{
	...
	*/
	...
	ixmlSchemaParseSchemaDefaults(pctxt, schema, docElem);
	ixmlSchemaParseSchemaTopLevel(pctxt, schema, docElem);
			 ...
			 int noNamespace)
{
    xmlChar *value;
    const xmlChar *cur, *end;
    const xmlChar *nsname = NULL, *location = NULL;
    int count = 0;
    int ret = 0;
...
/*    value = xmlNodeGetContent((xmlNodePtr) xsiAttr);    */
    value = ixmlNode_getNodeValueEx(xsiAttr);
    cur = value;
    do {
	if (noNamespace != 1) {
	    /*
...
/*	location = xmlDictLookup(vctxt->schema->dict, cur, end - cur); */
	...
	ret = xmlSchemaAssembleByLocation(vctxt, vctxt->schema,
	    xsiAttr->parentNode, nsname, location);
	if (ret == -1) {
...
*
*   Return Value:
*       matching attribute with the given local name and namespace URI
*       specified, NULL otherwise.
*
*=================================================================*/
IXML_Node *
ixmlHasNsProp( IN IXML_Node * pNode,
                            ...
                            IN DOMString localName )
{
    ...
    if( ( pNode == NULL ) || ( namespaceURI == NULL )
        || ( localName == NULL ) ) {
        return FALSE;
    }

    attrNode = pNode->firstAttr;
    while( attrNode != NULL ) 
	{
...
/*    attr = xmlHasNsProp(elem, BAD_CAST "schemaLocation", xmlSchemaInstanceNs); */
	...
	attr = ixmlHasNsProp(elem, (DOMString) xmlSchemaInstanceNs, (DOMString) "schemaLocation");
  ...
  /*  attr = xmlHasNsProp(elem, BAD_CAST "noNamespaceSchemaLocation", xmlSchemaInstanceNs); */
	...
	attr = ixmlHasNsProp(elem, (DOMString) xmlSchemaInstanceNs, (DOMString) "noNamespaceSchemaLocation");
...
xmlSchemaValidateCallback(xmlSchemaValidCtxtPtr ctxt,
                          const xmlChar * name ATTRIBUTE_UNUSED,
                          xmlSchemaTypePtr type, IXML_Node *node)
{
    xmlSchemaTypePtr oldtype = ctxt->type;
    IXML_Node *oldnode = ctxt->node;

#ifdef DEBUG_CONTENT
    xmlGenericError(xmlGenericErrorContext,
                    "xmlSchemaValidateCallback: %s, %s, %s\n",
                    name, type->name, node->nodeName);
#endif
    /*
				 ...
				 int checkNodes)
{
    ...
    */
	...
	do {
	    ...
	    case eCOMMENT_NODE:
	    /*case XML_XINCLUDE_START: TO BE FIXED*/
	    /*case XML_XINCLUDE_END: TO BE FIXED*/
		break;
		...
		*/
	    ...
	    case eINVALID_NODE:
							break;
#if 0
	    case XML_HTML_DOCUMENT_NODE: /*TO BE FIXED*/
	    case XML_DTD_NODE: /*TO BE FIXED*/
	    case XML_ELEMENT_DECL: /*TO BE FIXED*/
	    case XML_ATTRIBUTE_DECL: /*TO BE FIXED*/
	    case XML_ENTITY_DECL: /*TO BE FIXED*/
	    case XML_NAMESPACE_DECL: /*TO BE FIXED*/
#ifdef LIBXML_DOCB_ENABLED
	    case XML_DOCB_DOCUMENT_NODE: /*TO BE FIXED*/
#endif
		xmlSchemaVCustomErr(ctxt,
		    XML_SCHEMAV_INTERNAL,
		    /* XML_SCHEMAS_ERR_INVALIDELEM, */
    ...
    */
    ...
    for (i=0; i<ulAttrCount; i++)
    {
     	attr = ixmlNamedNodeMap_item(pAttribs,i);
   		strcpy(pAttribName, ixmlNode_getNodeName(attr));
   	if ((attr->namespaceURI == NULL) ||
            (!xmlStrEqual(attr->namespaceURI, xmlSchemaInstanceNs)) ||
            ((!xmlStrEqual(pAttribName, BAD_CAST "type")) &&
             (!xmlStrEqual(pAttribName, BAD_CAST "nil")) &&
             (!xmlStrEqual(pAttribName, BAD_CAST "schemaLocation")) &&
             (!xmlStrEqual
              (pAttribName, BAD_CAST "noNamespaceSchemaLocation")))) {
    ...
    while (attr != NULL) {
        if ((attr->namespaceURI == NULL) ||
            (!xmlStrEqual(attr->nanesoaceURI, xmlSchemaInstanceNs)) ||
            ((!xmlStrEqual(attr->nodeName, BAD_CAST "type")) &&
             (!xmlStrEqual(attr->nodeName, BAD_CAST "nil")) &&
             (!xmlStrEqual(attr->nodeName, BAD_CAST "schemaLocation")) &&
             (!xmlStrEqual
              (attr->nodeName, BAD_CAST "noNamespaceSchemaLocation")))) {
	    xmlSchemaVIllegalAttrErr(ctxt,
		XML_SCHEMAV_CVC_TYPE_3_1_1, attr);
	    ret = XML_SCHEMAV_CVC_TYPE_3_1_1;
        }
 ...
 */
static int
xmlSchemaValQNameAcquire(const xmlChar *value, IXML_Node *node,
			xmlChar **nsName, xmlChar **localName)
{
	...
	* string.
	*/
	local = ixmlSplitQName2(value, &prefix);
	if (local == NULL)
	    local = xmlStrdup(value);
	ixmlSearchNs(node->ownerDocument, node, prefix, (const xmlChar**)nsName);
 ...
 */
static int
xmlSchemaHasElemContent(IXML_Node *node)
{
    if (node == NULL)
	return (0);
				      ...
				      xmlSchemaElementPtr elemDecl)
{
    ...
    */
    ...
    attr = ixmlHasNsProp(elem, (DOMString)xmlSchemaInstanceNs, (DOMString)"nil");
    if (attr != NULL) {
/*	attrValue = xmlNodeGetContent((xmlNodePtr) attr);	*/
	...
	if (ret < 0) {
	    xmlSchemaVCustomErr(ctxt,
		XML_SCHEMAV_INTERNAL,
		 attr, (xmlSchemaTypePtr) elemDecl,
		"Internal error: xmlSchemaValidateElementByDeclaration, "
...
/*    attr = xmlHasNsProp(elem, BAD_CAST "type",  xmlSchemaInstanceNs); */
    attr = ixmlHasNsProp(elem, (DOMString)xmlSchemaInstanceNs, (DOMString)"type");
    if (attr != NULL) {
...
/*	attrValue = xmlNodeGetContent((xmlNodePtr) attr); */
	...
	ret = xmlSchemaValQNameAcquire(attrValue, attr->parentNode,
	    &nsName, &local);
	if (ret < 0) {
	    xmlSchemaVCustomErr(ctxt,
		XML_SCHEMAV_INTERNAL,
	...
	*/
	if (ret == 0) {
	    ...
	    */
		...
		textChild = ixmlDocument_createTextNode(elem->ownerDocument, (char*)elemDecl->value);
		if (textChild == NULL) {
		    xmlSchemaVCustomErr(ctxt,
			XML_SCHEMAV_INTERNAL,
			elem, actualType,
			...
			int rc = 0;
		    rc = ixmlNode_appendChild( elem, textChild);
	...
	if ((elemDecl->flags & XML_SCHEMAS_ELEM_FIXED) && (nilled == 0)) {
		...
		*/
		...
		if (actualType->contentType == XML_SCHEMA_CONTENT_MIXED) {
		    ...
		    */
		    value = ixmlNodeListGetString(elem->ownerDocument, elem->firstChild, 1);
		    ...
		    *
		    */
		    value = ixmlNodeListGetString(elem->ownerDocument,
		    								elem->firstChild, 1);
					   ...
					   IXML_Node *node)
{
	...
	*/
	...
	if (node->namespaceURI != NULL)
	    ...
	    node->nodeName, node->namespaceURI, NULL);
	else
	    decl = xmlHashLookup3(ctxt->schema->elemDecl, node->nodeName, NULL, NULL);
	if (decl != NULL) {
	    ctxt->node = node;
	...
	child = node->firstChild;
	do {
	    if (child->nodeType == eELEMENT_NODE) {
				  ...
				  xmlSchemaTypePtr type)
{
    ...
    */
	...
	if (cur->nodeType == eELEMENT_NODE) {
	    ...
	    */
		...
		decl = xmlHashLookup3(ctxt->schema->elemDecl,
		    cur->nodeName, cur->namespaceURI, NULL);
	    else
		decl = xmlHashLookup3(ctxt->schema->elemDecl, cur->nodeName, NULL, NULL);
	    if (decl != NULL) {
		ctxt->node = cur;
		ret = xmlSchemaValidateElementByDeclaration(ctxt, decl);
		ctxt->node = top;
	...
	*/
        if ((skipContent == 0) && (cur->firstChild != NULL)) {
	    /* deep first */
	    cur = cur->firstChild;
	} else if ((cur != top) && (cur->nextSibling != NULL)) {
	    /* then siblings */
	    cur = cur->nextSibling;
	} else if (cur != top) {
	    ...
	    while (cur != top) {
		    ...
		    cur = cur->parentNode;
		if ((cur != top) && (cur->nextSibling != NULL)) {
		    cur = cur->nextSibling;
		    break;
		}
		if (cur->parentNode == NULL) {
				      ...
				      int valSimpleContent)
{
    ...
    * STREAM: Attribute nodes are processed.
    */
    xmlSchemaRegisterAttributes(ctxt, elem->firstAttr);
	...
	case XML_SCHEMA_CONTENT_MIXED:
	    ...
	    */
		...
		if (child->nodeType == eELEMENT_NODE) {
		    ...
		    */
		    (((child->nodeType == eTEXT_NODE) && (!IS_BLANK_NODE(child))) ||
		     (child->nodeType == eENTITY_NODE) ||
		     (child->nodeType == eENTITY_REFERENCE_NODE) ||
		     (child->nodeType == eCDATA_SECTION_NODE))) {
		    /*
	...
	if (valSimpleContent == 1) {
		...
		*/
		if (elem->firstChild == NULL)
		    ...
		    value = ixmlNode_getNodeValueEx(elem);
 ...
 */
static int
xmlSchemaValidateAttributes(xmlSchemaValidCtxtPtr ctxt, IXML_Node *elem, xmlSchemaTypePtr type)
{
    const xmlChar *nsURI;
    int ret;
    IXML_Node *attr; /* An attribute on the element. */
    const xmlChar *defValue;
    xmlSchemaValPtr defVal;
    int fixed;
	...
	printf("attr use - use: %d\n", attrDecl->occurs);
	    ...
	    /* TODO: Can this ever happen? */
            ...
            if (attrDecl->ref != NULL) {
                if (!xmlStrEqual(attr->nodeName, attrDecl->ref))
                    continue;
                if (attr->namespaceURI != NULL) {
                    if ((attrDecl->refNs == NULL) ||
                        (!xmlStrEqual(attr->namespaceURI, attrDecl->refNs)))
                        continue;
                } else if (attrDecl->refNs != NULL) {
                    continue;
                }
            } else {
                if (!xmlStrEqual(attr->nodeName, attrDecl->name))
                    continue;
                /*
    ...
    */
	...
	while (curState != NULL) {
		...
		if (curState->attr->namespaceURI != NULL)
		    nsURI = curState->attr->namespaceURI;
		else
		    nsURI = NULL;
    ...
    */
	...
	while ((curState != NULL) && (curState != ctxt->attrTop->next)) {
		...
		if (curState->state == XML_SCHEMAS_ATTR_MISSING) {
			...
			xmlSchemaVCustomErr(ctxt,
			    XML_SCHEMAV_CVC_AU,
			     attr, (xmlSchemaTypePtr) attrDecl,
    ...
    */
	...
	while (curState != NULL) {
	    ...
	    */
		...
		if (attrDecl->targetNamespace != NULL)
		{
			ixmlNode_setAttributeNS( elem, (DOMString)attrDecl->targetNamespace,
							 (DOMString)attrDecl->name, (DOMString)curState->value);
	   	}
		else
		{
			/*check for return value*/
			ixmlNode_setAttribute( elem, (char*)attrDecl->name, (char*)curState->value);
		}
		...
		ns = xmlSearchNsByHref(elem->ownerDocument, elem,
		    attrDecl->targetNamespace); /*TO BE FIXED*/
		if (ns == NULL) {
		    xmlChar prefix[12];
		    int counter = 1;
		    ...
		    */
		    T_snprintf((char *) prefix, sizeof(prefix), "p");
		    /*
		    ...
		    */
			int retval = 0;
			xmlChar *nsStr = NULL;
		    /*ns = ixmlSearchNs(elem->ownerDocument, elem, BAD_CAST prefix);*/
			retval = ixmlSearchNs(elem->ownerDocument, elem, BAD_CAST prefix, &nsStr);
		    ...
		    while (nsStr != NULL) {
				...
				T_snprintf((char *) prefix, sizeof(prefix), "p%d", counter++);
		    ...
		    if (nsStr == NULL) {
				ns = xmlNewNs(ctxt->validationRoot,
			    	attrDecl->targetNamespace, BAD_CAST prefix); /*TO BE FIXED*/
				xmlNewNsProp(elem, ns, attrDecl->name, curState->value);
		    } /*nsStr == NULL*/
		} else {
		    xmlNewNsProp(elem, ns, attrDecl->name, curState->value);/*TO BE FIXED*/
		}
	    }
#endif
	    curState = curState->next;
...
xmlSchemaValidateElement(xmlSchemaValidCtxtPtr ctxt)
{
	...
	*/
    ...
    if (ret != -1) {
	if (ctxt->node->namespaceURI != NULL)
	    elemDecl = xmlSchemaGetElem(ctxt->schema, ctxt->node->nodeName,
		ctxt->node->namespaceURI);
	else
	    elemDecl = xmlSchemaGetElem(ctxt->schema, ctxt->node->nodeName, NULL);
 ...
 */
int
xmlSchemaValidateOneElement(xmlSchemaValidCtxtPtr ctxt, IXML_Node *elem)
{
    if ((ctxt == NULL) || (elem == NULL) || (elem->nodeType != eELEMENT_NODE))
	return (-1);
 ...
 */
static int
xmlSchemaValidateDocument(xmlSchemaValidCtxtPtr ctxt, IXML_Document *doc)
{
    IXML_Node *root = NULL;
 ...
 */
int
xmlSchemaValidateDoc(xmlSchemaValidCtxtPtr ctxt, IXML_Document *doc)
{
    int ret;
