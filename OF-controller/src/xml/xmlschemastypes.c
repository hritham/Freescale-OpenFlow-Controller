...
/*Include Intoto specific timezone*/
#include "tzgif.h"
...
/* Intoto Date and Time */
static xmlSchemaTypePtr xmlSchemaTypePSPDate = NULL;
static xmlSchemaTypePtr xmlSchemaTypePSPTime = NULL;
static xmlSchemaTypePtr xmlSchemaTypePSPTimeZone = NULL;
 ...
 */
static void
xmlSchemaTypeErrMemory(IXML_Node *node, const char *extra)
{
	printf("\nOut of Memory error XML_FROM_DATATYPE = %d \n", XML_FROM_DATATYPE);
#if 0
...
xmlSchemaInitBasicType(const char *name, xmlSchemaValType type,
    ...
    */
	...
	case XML_SCHEMAS_PSP_DATE:
	case XML_SCHEMAS_PSP_TIME:
	case XML_SCHEMAS_PSP_TIMEZONE:
...
xmlSchemaInitTypes(void)
{
    ...
    */
	...
	xmlSchemaTypePSPDate	= xmlSchemaInitBasicType("pspdate",
                                                   XML_SCHEMAS_PSP_DATE,
						   xmlSchemaTypeAnySimpleTypeDef);		   

	xmlSchemaTypePSPTime	= xmlSchemaInitBasicType("psptime",
                                                   XML_SCHEMAS_PSP_TIME,
						   xmlSchemaTypeAnySimpleTypeDef);		   

	xmlSchemaTypePSPTimeZone = xmlSchemaInitBasicType("psptimezone",
                                                   XML_SCHEMAS_PSP_TIMEZONE,
						   xmlSchemaTypeAnySimpleTypeDef);		   
    /*
     * derived datatypes
     */
...
static int PSPSchemaValidateDate(const xmlChar *value)
{
	unsigned int ulMm,ulDd,ulYy;
	xmlChar *pArg;
	xmlChar *pTmp1, *pTmp2, *pTmp;

	pTmp1 = pTmp2  = NULL;
	pTmp1 = pArg = (xmlChar *)value;
	pArg = (xmlChar *)T_strchr(value, '-');

	if(!pArg)
	{
#ifdef XMLDEBUG
		printf("1.Invalid date. enter date in mm-dd-yyyy format\r\n");
#endif
		return (-1);
	}
	...
	*pArg++ = 0;
	...
	pTmp = pTmp2 = pArg;
	pArg =(xmlChar *)T_strchr((xmlChar *)pTmp2, '-');
	if(!pArg)
	{
#ifdef XMLDEBUG
		printf("2.Invalid date. enter date in mm-dd-yyyy format\r\n");
#endif		  
		return (-1);
	}
	*pArg++ = 0;
	...
	if(ulMm <1 || ulMm>12 || ulDd < 1 || ulDd > 31)
	{
#ifdef XMLDEBUG
	...
	if( (ulYy < 1975) || (ulYy > 2038) )
	{
#ifdef XMLDEBUG
		printf("System Year can be between 1975 and 2038\r\n");
#endif
		return (-1);
	}
	if( ulYy == 2038 )
	{
		...
		{
#ifdef XMLDEBUG
			printf("System date allowed is up to Jan 2038\r\n");
#endif
			return (-1);
		}
		if( ulDd > 18 )
		{
#ifdef XMLDEBUG
			printf("System date allowed is up to 18 Jan 2038\r\n");
#endif
			return (-1);
		}
	}
...
static int PSPSchemaValidateTime(const xmlChar *value)
{
	unsigned int ulSs,ulHh,ulMn;
	xmlChar *pArg;
	xmlChar *pTmp1, *pTmp2, *pTmp;
	t_bool8 bAmOrPmSet,bAmSet,bPmSet;

	bAmOrPmSet = bAmSet = bPmSet = FALSE;
	ulHh = ulMn = ulSs = 0;
	pTmp1 = pTmp2  = NULL;
	pTmp1 = pArg = (xmlChar *)value;
	pArg = (xmlChar *)T_strchr((xmlChar *)value, ':');
	if(pArg)
	*pArg++ = 0;
	pTmp = pTmp1;
	while(*pTmp)
	...
	{
		pTmp = pTmp2 = pArg;
		pArg = (t_char8 *)T_strchr((t_char8 *)pTmp2, ':');
	...
	{
#ifdef XMLDEBUG
		printf("4.Invalid time\r\n");
#endif
		return (-1);
	}

	if(*pTmp && (toupper(*pTmp) == 'A' && pTmp[1] == 0))
	{
		bAmSet = bAmOrPmSet = TRUE;
	}
	else if(*pTmp &&(toupper(*pTmp) == 'P' && pTmp[1] == 0))
	{
		bPmSet = bAmOrPmSet = TRUE;
	}
	else if(*pTmp)
	{
#ifdef XMLDEBUG
		printf("5.Invalid time\r\n");
#endif
		return (-1) ;
	}
	*pTmp =0;
	...
	if(ulSs > 59 || ulMn > 59 || ulHh > 23)
	{
#ifdef XMLDEBUG
		printf("6.Invalid time\r\n");
#endif
		return (-1) ;
	}
...
static int PSPSchemaValidateTimeZone(const xmlChar *value)
{
	t_int32 lHh, lMn,lTimeDiff;
	t_int32 iIndex;
#ifdef XMLDEBUG
	t_char8 ErrMsg[] = "Format of time must be +/-hh:mm-<ZONE>\n\r";
#endif
	t_char8 *pArg;
	t_uchar8 cAbbrv[32 + 1];
	t_bool8  bPlus = FALSE;

	T_memset(cAbbrv,0,32+1); 
	lHh = lMn = 0;
	pArg = (t_char8 *)value;
	...
	{
#ifdef XMLDEBUG
		printf("Time zone must start with + or -\n\r");
		printf("%s",ErrMsg);
#endif
		return (-1);
	}
	...
	{
#ifdef XMLDEBUG
		printf("Hours and minutes must be seperated by colon(:)\n\r");
		printf("%s",ErrMsg);
#endif
		return (-1);
	}
	...
	{
#ifdef XMLDEBUG
		printf("Hours and minutes must be digits between 0 and 9\n\r");
		printf("%s",ErrMsg);
#endif
		return (-1);
	}
	...
	{
#ifdef XMLDEBUG
		printf("Hours cannot be more than 14 or minutes cannot be other than 0 and 30\r\n");
		printf("%s",ErrMsg);
#endif
		return (-1);
	}
	...
	if(bPlus)
		lTimeDiff = (lHh*60 + lMn);
	else
		lTimeDiff = -(lHh*60 + lMn);

	of_strcpy(cAbbrv,&pArg[7]);

	iIndex = TZGetIndexByDiffandAbbr(lTimeDiff,cAbbrv);

	if( iIndex == T_FAILURE )
	{
#ifdef XMLDEBUG
		printf("Invalid time zone\r\n\tValid Zones are\n\r");
		printf("\t +/-hh Where -13 > hh < 14\n\r");
		printf("\t +/-hh:30 Where hh = 11,10,9,8,6,5,4,3\n\r");
#endif
		return (-1);
	}
	return 0;
...
xmlSchemaValAtomicListNode(xmlSchemaTypePtr type, const xmlChar *value,
	                   xmlSchemaValPtr *ret, IXML_Node *node) {
...
xmlSchemaValAtomicType(xmlSchemaTypePtr type, const xmlChar * value,
                       xmlSchemaValPtr * val, IXML_Node *node, int flags)
{
    xmlSchemaValPtr v;
    xmlChar *norm = NULL;
    int ret = 0;
	...
	case XML_SCHEMAS_ANYSIMPLETYPE:
        ...
        case XML_SCHEMAS_DATE:
        case XML_SCHEMAS_DATETIME:
		/* replaced by Intoto Date validation routines */
            	/*ret = xmlSchemaValidateDates(type->builtInType, value, val);*/
            ret=-1;
            break;

        case XML_SCHEMAS_PSP_DATE:
		ret = PSPSchemaValidateDate(value);
        	break;

        case XML_SCHEMAS_PSP_TIME:
		ret = PSPSchemaValidateTime(value);
        	break;

        case XML_SCHEMAS_PSP_TIMEZONE:
		ret = PSPSchemaValidateTimeZone(value);
        	break;
        ...
        case XML_SCHEMAS_LANGUAGE:
            ...
            goto return1;
        case XML_SCHEMAS_NMTOKEN:
            if (ixmlValidateNMToken(value, 1) == 0) {
                if (val != NULL) {
        ...
        case XML_SCHEMAS_QNAME:{
                ...
                if ((ret == 0) && (node != NULL)) {
                    ...
                    if (prefix != NULL) {
                        ixmlSearchNs(node->ownerDocument, node, prefix, (const xmlChar**)(&uri));
                    }
		...
		goto return0;
#if 0
        	/*IXML: For the time being lets just bring in  URI.c */
        ...
        case XML_SCHEMAS_HEXBINARY:{
              ...
              /*  if (cur == NULL)
                ...
                if (val != NULL) {
                    ...
                    if (cur == NULL) {
		        /*
		         * IXML: To be taken care at IXML API Level
 ...
 */
int
ixmlValidateName(const xmlChar *value, int space) {
    const xmlChar *cur = value;
    int c,l;
 ...
 */
int
ixmlValidateNMToken(const xmlChar *value, int space) {
    const xmlChar *cur = value;
    int c,l;
 ...
 */
int
ixmlValidateNCName(const xmlChar *value, int space) {
    const xmlChar *cur = value;
    int c = 0,l = 0;
...
xmlSchemaCompareValues(xmlSchemaValPtr x, xmlSchemaValPtr y) {
	...
	case XML_SCHEMAS_DECIMAL:
        ...
        case XML_SCHEMAS_FLOAT:
        case XML_SCHEMAS_DOUBLE:
		printf("\nxmlSchemaCompareValues: Unsupported Float type schema found !!\n");
		return(-2);
#if 0
