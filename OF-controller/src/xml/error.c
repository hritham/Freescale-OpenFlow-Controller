               ...
               xmlGenericErrorFunc channel, void *data)
{
    ...
    domain = err->domain;
    level = err->level;
    node = (IXML_Node *)err->node;

    if (code == XML_ERR_OK)
        return;

    if ((node != NULL) && (node->nodeType == eELEMENT_NODE))
        name = node->nodeName;
	      ...
	      const char *msg, ...)
{
    ...
    IXML_Node *node = (IXML_Node *) nod;
    char *str = NULL;
    xmlParserInputPtr input = NULL;
    xmlErrorPtr to = &xmlLastError;
    IXML_Node *baseptr = NULL;
...
/*	if ((node->doc != NULL) && (node->doc->URL != NULL))*/
	if (node->ownerDocument!= NULL)
	    baseptr = node;
	for (i = 0;
	     ((i < 10) && (node != NULL) && (node->nodeType != eELEMENT_NODE));
	     i++)
...
/*	    to->file = (char *) xmlStrdup(baseptr->doc->URL);*/
	to->file = (char *) xmlStrdup(baseptr->ownerDocument->n.nodeName);
	file = to->file;
    }
    to->line = line;
    if (str1 != NULL)
 ...
 */
void
__xmlSimpleError(int domain, int code, IXML_Node *node,
