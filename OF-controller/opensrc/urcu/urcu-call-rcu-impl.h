...
#ifdef URCU_DEBUG
                                rcu_loops++;
                                if((rcu_loops % 10000) == 0)
                                  printf("%s::rcu_loops %d threadid %d\n", 
                                              __FUNCTION__,
                                              rcu_loops, pthread_self());
#endif
				rhp->func(rhp);
				cbcount++;
			} while (cbs != NULL);
			       ...
			       int cpu_affinity)
{
	...
	*crdpp = crdp;
	if (pthread_create(&crdp->tid, NULL, call_rcu_thread, crdp) != 0) {
        ...
        printf("%s(%d):************Call RCU tid:%p **************\r\n",__FUNCTION__,__LINE__,(void *)crdp->tid);
}
