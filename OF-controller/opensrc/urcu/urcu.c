...
#ifdef RCU_MB
void *urcu_call_rcu_thread(void *arg, int cpu_affinity)
{
  struct call_rcu_data *crdp;

  printf("%s::arg %p, affinity %d\n", __FUNCTION__, arg, cpu_affinity);
  crdp = (struct call_rcu_data *)arg;
  crdp->tid = pthread_self();
  crdp->cpu_affinity = cpu_affinity;
  return(call_rcu_thread(arg));
}
#endif
