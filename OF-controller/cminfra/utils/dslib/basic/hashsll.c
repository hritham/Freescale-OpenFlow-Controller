 ...
 *              Hash Table single linked list operations.
 * 
 */
...
/***************************************************************************
 * Function Name : cm_hash_table_create
 * Description   : Creates hash table and initializes the singly linked list
 *                 buckets
...
{
  struct cm_hash_sll_table   *pHashTab = NULL;
  uint32_t          ulMemsize = 0;
  uint32_t          ulKey = 0;

  /* calculate the memory required */
  ulMemsize = sizeof (struct cm_hash_sll_table) +
              ((ulBuckets -1) * sizeof (UCMHashBucketList_t));
  
  pHashTab = (struct cm_hash_sll_table *) of_malloc (ulMemsize);
  if (pHashTab == NULL)
  {
